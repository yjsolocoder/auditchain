...
（payload 仍密封、不拆开），附一份覆盖全部条目的共享紧凑批量证明，并由签发方
用查询密钥当场逐条解封比对、把得出的命中索引按升序一并记入回执。持钥核验方
无需持有日志，先认证条目真伪，再自行逐条解封比对，重算出的命中集与回执记录
逐项一致才判真——范围内少列、多列或伪造命中都无法隐瞒：
...
verify_full_encrypted_search_receipt(receipt, key)                # True：离线解封比对 + 命中集一致
verify_full_encrypted_search_receipt(receipt, wrong_key)          # False：记录了命中时错误密钥无法复现
log.full_encrypted_search_receipt("secret event", key, 1, 3, size=3)  # 半开范围 [1, 3) + 前 3 条快照
```

回执为冻结的
`FullEncryptedSearchReceipt(version=1, hash_name, size, root, query, start,
stop, items, proof, hits)`，支持位置构造、按全部十个字段相等：记录算法、快照
尺寸与 Merkle 根、规范化查询值、半开范围 `[start, stop)`、该范围内**每一个**
`Entry`（按绝对索引严格升序，恰为 `start .. stop-1` 逐条各一份）、一份覆盖
它们的共享紧凑批量证明（与 `batch_inclusion_proof` 同一布局）以及升序的命中
索引 `hits`。查询值只收 `bytes` 或 `str`（后者按 UTF-8 规范化），其他类型抛
`TypeError`；密钥只收 32 字节 `bytes`（类型错抛 `TypeError`，长度不符抛
`ValueError`）；其余二进制字段——`root`、每个条目的 `payload` /
`previous_hash` / `entry_hash` 与每个共享证明节点——只接受精确的 `bytes`，
`bytearray` 与 `memoryview` 抛 `TypeError`。范围默认 `[retain_from, size)`，
`size` 默认当前长度且快照须可重建。空范围与空快照照常签发核验，此时条目、
证明与命中都是空元组。签发只读、可重复，不改变条目、`head`、认证状态、密文
定位索引、Merkle 根或证明；范围/尺寸类型错抛 `TypeError`、越界或快照不可
重建抛 `ValueError`。

离线核验逐条重算 `entry_digest`、用共享批量证明（`verify_batch_inclusion`）
核对快照根（空快照只认规范空树根），再以传入密钥逐条尝试解封：成功且明文等于
...
- `FullEncryptedSearchReceipt(version, hash_name, size, root, query, start,
  stop, items, proof, hits)` — 不可变的完整加密检索回执，把一次按原明文定位的
  **全覆盖**结果（算法、快照尺寸与 Merkle 根、规范化查询值、半开范围
  `[start, stop)`、该范围内每一个仍密封的 `Entry`、一份共享紧凑批量证明与
  签发方当场记录的升序命中索引）绑成一件可落盘制品，按全部十个字段相等、
  支持位置构造；`version` 恒为 `1`，`query` 只收 `bytes` 或 `str`（后者按
  UTF-8 规范化后存储），范围满足 `0 <= start <= stop <= size`，`items` 按
  绝对索引严格升序且恰为 `start .. stop-1` 逐条各一份（覆盖不全、重复或
  乱序均非法），`proof` 为覆盖全部所列条目的共享紧凑批量证明（与
  `batch_inclusion_proof` 同一布局，空范围时与 `items`、`hits` 同为 `()`），
  `hits` 为严格升序且落在记录范围内的命中索引（不合法即拒）；持钥核验方
  认证条目后自行逐条解封比对，重算命中集与 `hits` 逐项一致才判真。
  `query` 以外的二进制字段（`root`、每个条目的 `payload` / `previous_hash` /
...
- `AuditLog(*, key=None, hash_name="sha256")` — 传入非空 `bytes` 类型 `key` 开启前向安全认证，
  ...
  - `full_encrypted_search_receipt(query, key, start=None, stop=None, size=None)` —
    把一次按原明文定位的查找固化为可离线核验**完整性**的冻结
    `FullEncryptedSearchReceipt`：沿用 `find_encrypted` 的口径，在半开范围
    `[start, stop)`（默认 `[retain_from, size)`）内，`size` 默认当前长度且
    快照须可重建；回执记录算法、快照尺寸与 Merkle 根、规范化查询值、范围、
    该范围内**每一个**条目（payload 仍密封，按绝对索引严格升序逐条各一份）、
    一份覆盖全部条目的共享紧凑批量证明，以及签发方用查询密钥当场逐条解封
    比对得出的升序命中索引 `hits`（空范围与空快照时三者均为 `()`），不记录
    密钥与明文。`query` 只收 `bytes` 或 `str`（后者按 UTF-8 规范化），其他
    类型抛 `TypeError`；`key` 只收 32 字节 `bytes`（类型错抛 `TypeError`，
...
- `verify_full_encrypted_search_receipt(receipt, key)` — 无需持有日志即可验证
  ...
  `decrypt_entry` 逐条尝试解封并把恢复明文与规范化查询值逐字节比较；重算出的
