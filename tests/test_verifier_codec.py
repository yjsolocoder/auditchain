...
class EncodeVerifierTest(unittest.TestCase):
    ...
    def test_any_nonempty_key_width_roundtrips(self):
        for key in (b"k", b"x" * 32, b"y" * 100):
    ...
    def test_encode_is_deterministic(self):
        material = _material()
        self.assertEqual(
            encode_verifier(material), encode_verifier(material)
        )
...
class DecodeVerifierTest(unittest.TestCase):
    ...
    def test_roundtrip_any_key_width(self):
        self.roundtrip(Verifier(b"k", "sha256"))
        self.roundtrip(Verifier(b"x" * 100, "sha3_256"))
    ...
    def test_unknown_hash_algorithm(self):
        data = MAGIC + u64(1) + blob(b"not-a-hash") + blob(b"k")
        with self.assertRaises(ValueError):
            decode_verifier(data)
    ...
    def test_invalid_utf8_hash_name(self):
        data = MAGIC + u64(1) + blob(b"\xff\xfe") + blob(b"k")
        with self.assertRaises(ValueError):
            decode_verifier(data)
    ...
    def test_tampered_key_still_decodes_and_fails_verification(self):
        data = MAGIC + u64(1) + blob(b"sha256") + blob(b"wrong-key")
        decoded = decode_verifier(data)
        self.assertEqual(decoded, Verifier(b"wrong-key", "sha256"))
        self.assertFalse(verify_auth(self.log.entry(0), self.tag0, decoded))

    def test_tampered_hash_name_still_decodes_and_fails_verification(self):
        # sha3_256 matches sha256's digest width, so verification reaches a
        # verdict instead of raising on the width check.
        data = MAGIC + u64(1) + blob(b"sha3_256") + blob(self.material.key)
        decoded = decode_verifier(data)
        self.assertEqual(decoded, Verifier(self.material.key, "sha3_256"))
        self.assertFalse(verify_auth(self.log.entry(0), self.tag0, decoded))
