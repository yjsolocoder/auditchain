...
MAGIC = b"auditchain/signed-encrypted-search/v1\0"
...
class DecodeSignedEncryptedSearchReceiptTest(unittest.TestCase):
    ...
    def test_roundtrip_empty_log(self):
        self.roundtrip(
            AuditLog().signed_encrypted_search_receipt("hit", KEY, _SEED_A)
        )
    ...
    def test_roundtrip_after_prune(self):
        ...
        for i in range(6):
            record = f"r{i}"
            # setUp's log already used nonces 0..6.
            nonce = bytes([7 + i]) * 12
            self.log.encrypt(record, KEY, nonce=nonce)
            twin.encrypt(record, KEY, nonce=nonce)
        self.log.prune(2, self.log.seal(2))
        self.roundtrip(
            self.log.signed_encrypted_search_receipt("r4", KEY, _SEED_A)
        )

    def test_roundtrip_alternate_hash(self):
        for hash_name in ("sha512", "sha3_256"):
            log = make_log(("a", "hit", "b"), hash_name=hash_name)
            self.roundtrip(
                log.signed_encrypted_search_receipt("hit", KEY, _SEED_A)
            )
    ...
    def test_wrong_key_decodes_but_verifies_false(self):
        bundle = self.log.signed_encrypted_search_receipt("hit", KEY, _SEED_B)
        decoded = decode_signed_encrypted_search_receipt(
            encode_signed_encrypted_search_receipt(bundle)
        )
        self.assertFalse(
