...
# a u64 and three u64-length-prefixed blobs in a fixed order — the complete
# canonical encode_continuations output of the receipts, then the complete
# canonical encode_signed_root output of the start and end anchors — with
# nothing else.
...
class AnchoredContinuationChain:
    """A continuation chain pinned to its two expected endpoint checkpoints.

    Bundles a non-empty tuple of :class:`SignedAuthAuditContinuation`
    receipts with the two :class:`SignedRoot` checkpoints the chain is
    expected to span — the ``old`` checkpoint of the first segment and the
    ``new`` checkpoint of the last — so the anchored chain can be persisted
    and shipped as one artifact (see :func:`encode_anchored_continuations`)
    and later diagnosed as one artifact (see
    :func:`inspect_anchored_continuations`), without holding the log and
    without any new signing domain:

    - ``receipts``: the non-empty tuple of chained
      :class:`SignedAuthAuditContinuation` receipts, in chain order,
    - ``start``: the :class:`SignedRoot` the first segment's
      ``consistency.old`` must equal,
    - ``end``: the :class:`SignedRoot` the last segment's
    ...
    a wrong element type or a non-:class:`SignedRoot` anchor raises
    TypeError, an empty tuple raises ValueError. Whether the receipts
    actually form one continuous chain between the two anchors is left to
...
def encode_anchored_continuations(bundle: Any) -> bytes:
    ...
    1), then the chain blob, the start anchor blob and the end anchor blob —
    nothing may be omitted, reordered or appended. ``C`` is byte-for-byte
    the complete canonical output of :func:`encode_continuations` over
    ``bundle.receipts`` and ``S`` / ``E`` the complete canonical output of
    :func:`encode_signed_root` over ``bundle.start`` / ``bundle.end``; the
    framing introduces no new signing message and is read-only.

    ``bundle`` must be an :class:`AnchoredContinuationChain` — anything else
    (including fields of the wrong type written past the frozen constructor)
...
def decode_anchored_continuations(data: Any) -> AnchoredContinuationChain:
    ...
    each consumed whole with no trailing bytes. The chain blob is handed to
    :func:`decode_continuations` and the anchor blobs to
    ...
    relations are not checked here — only
    :func:`inspect_anchored_continuations` confirms the chain spans exactly
    from ``start`` to ``end``. The returned bundle's fields equal the
    originally encoded ones, and re-encoding reproduces the original bytes
    exactly.
    """
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
...
def inspect_anchored_continuations(
    ...
    """Diagnose an :class:`AnchoredContinuationChain` as one artifact.

    The persisted-bundle counterpart of :func:`inspect_anchors`: an offline
    party holding only the pre-trusted key and the bundle diagnoses in one
    call whether the receipts form one internally continuous chain spanning
    **exactly** from the bundle's ``start`` anchor to its ``end`` anchor.
    The diagnosis is exactly
    ``inspect_anchors(bundle.receipts, key, bundle.start, bundle.end)`` — the
    same first-failure order (``"verify"``, ``"growth"``, ``"duplicate"``,
    ``"link"`` before the anchor codes, ``"start"`` before ``"end"``), the
    same frozen :class:`ContinuationChainReport`, equally read-only, holding
    neither the log nor any checkpoint history and introducing no new
    Ed25519 or HMAC signing domain.

    ``bundle`` must be an :class:`AnchoredContinuationChain` — anything else
    raises TypeError. ``key`` must be a 32-byte ``bytes`` Ed25519 public
    key: a non-``bytes`` value (including ``bytearray``) raises TypeError
    and a ``bytes`` value of another length raises ValueError. Fields of the
    wrong type written past the frozen constructor and nested structural
    violations on a structurally illegal receipt propagate exactly the
    TypeError/ValueError of :func:`inspect_anchors`.
    """
    if not isinstance(bundle, AnchoredContinuationChain):
        raise TypeError("bundle must be an AnchoredContinuationChain")
