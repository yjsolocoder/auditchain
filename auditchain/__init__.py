...
# No codec exists for the full encrypted search receipt yet; the version
# constant is defined here so the receipt class and its issuer/verifier share
# exactly one source of truth, as for the other receipt families.
...
class FullEncryptedSearchReceipt:
    ...
    - ``items``: every :class:`Entry` of the searched range, one per
      absolute index in strictly ascending order — exactly the indices
      ``start, start + 1, ..., stop - 1``, so an incomplete coverage, a
      duplicate or an out-of-order entry is rejected at construction. Each
      entry's payload is the still-sealed encrypted-entry envelope (or a
      plain payload for entries appended without encryption); the receipt
      never records the key or any plaintext,
    - ``proof``: the single shared compact batch inclusion proof (as
      produced by :meth:`AuditLog.batch_inclusion_proof`) covering all of
      ``items`` within the snapshot,
    - ``hits``: the ascending absolute indices the issuer recovered by
      unsealing every listed entry with the query key and keeping exactly
      those whose plaintext equals the normalized query.

    Unlike :class:`EncryptedSearchReceipt`, which only attests that the
    listed hits are genuine, this receipt carries the whole searched range:
    once the entries are authenticated against the snapshot root, the
    verifier's own unsealing pass yields the complete hit set, which must
    equal the recorded ``hits`` — a concealed hit is a structural
    impossibility rather than a silent omission. An empty range (and any
    empty snapshot) carries ``items == ()``, ``proof == ()`` and
    ``hits == ()``.

    Instances are immutable, may be built positionally and compare by all
    ten fields. The constructor fixes only types, widths, ordering,
    coverage, ranges and the shape of ``hits`` — whether the entry digests
    and the shared proof actually rebuild ``root``, and whether the
    recorded hits match what the key recovers, is left to
    :func:`verify_full_encrypted_search_receipt`, so a tampered receipt is
    still constructible. Every binary field (``root``, each entry's
    ``payload`` / ``previous_hash`` / ``entry_hash`` and every shared-proof
    node) must be exact ``bytes``: ``bytearray`` and ``memoryview`` are
    rejected rather than copied, so a received receipt never silently
...
class AuditLog:
    ...
    def full_encrypted_search_receipt(
        ...
        """Issue an offline :class:`FullEncryptedSearchReceipt` proving completeness.
        ...
        ``size`` entries, together with a single shared compact batch
        inclusion proof covering all of them, and records the ascending
        absolute indices the issuer itself recovered by unsealing each entry
        with ``key`` and keeping those whose plaintext equals the normalized
        ``query``. :func:`verify_full_encrypted_search_receipt` can therefore
        check offline — without holding the log — that the listed entries are
        exactly the range's content and that its own unsealing pass
        reproduces the recorded hit set: no hit inside the range can be
        concealed or forged. Each entry's payload remains the sealed
        encrypted-entry envelope; the key and the plaintext are never
        recorded.
        ...
        out-of-range values or an unrebuildable snapshot ValueError. Plain
        entries and entries sealed under another key never hit, and a valid
        32-byte key that simply was not an append key yields ``hits == ()``.
        An empty range — and any empty snapshot — yields ``items == ()``,
        ``proof == ()`` and ``hits == ()``. The call is read-only and may be
        repeated at will: entries, head, authentication state, the encrypted
        locator index, Merkle roots and proofs are left untouched.
        """
        if isinstance(query, str):
            material = query.encode("utf-8")
        elif isinstance(query, bytes):
        ...
        for entry in items:
            try:
                ...
                # A plain entry, an envelope sealed under another key or an
                # envelope that does not authenticate is simply not a hit.
                continue
            if plaintext == material:
...
def verify_full_encrypted_search_receipt(receipt: Any, key: Any) -> bool:
    ...
    receipt carries every entry of the searched range ``[start, stop)`` —
    the constructor rejects an incomplete coverage, duplicates and
    out-of-order indices — the caller's own unsealing pass then yields the
    complete hit set: each entry is decrypted with ``key`` via
    :func:`decrypt_entry` and the recovered plaintext compared byte-for-byte
    against the normalized query. Verification returns True only when the
    authenticity checks hold *and* the recomputed hit set equals the
    receipt's recorded ``hits`` exactly, so unlike
    :func:`verify_encrypted_search_receipt` a concealed hit is a structural
    failure, not a silent omission.

    An empty range (``items == ()``, ``proof == ()`` and ``hits == ()``)
    attests no content; an empty snapshot (``size == 0``) additionally only
    accepts the canonical empty-tree root. Plain entries, envelopes sealed
    under another key and envelopes that fail to decrypt are simply not
    hits — they never fail verification by themselves. A structurally valid
    receipt whose entry content, proof or root does not match returns False,
    as does a receipt whose recorded hits cannot be reproduced with ``key``
    — in particular a receipt recording hits returns False under a wrong
    key, while a zero-hit receipt is decided by authenticity alone.
    Tampered content, proofs or roots return False rather than raising.

    ``key`` must be exactly 32 ``bytes`` — a non-bytes value raises TypeError
    and a wrong length raises ValueError. A receipt that is not a
    :class:`FullEncryptedSearchReceipt` raises TypeError; a receipt whose
    frozen fields were bypassed into an illegal shape (wrong types, an
    unknown hash algorithm, an out-of-range range or size, digest-width
    mismatches, non-ascending or incomplete items, illegal hit indices)
    raises the same TypeError or ValueError construction would, and a proof
    ...
    for entry in checked.items:
        try:
            ...
            # A plain entry, a foreign-key envelope or an AEAD
            # authentication failure is not a hit, not an error.
            continue
