...
# canonical encode_signed_auth_audit_bundle and encode_signed_consistency
# bytes, in that order and with nothing else. It introduces no domain of its
# own: it only persists the two frozen packages, and the bundle↔consistency
# linkage remains verify_signed_auth_audit_continuation's job.
...
def encode_signed_auth_audit_continuation(receipt: Any) -> bytes:
    ...
    canonical output of :func:`encode_signed_auth_audit_bundle` over
    ``receipt.bundle`` and ``C`` the complete canonical output of
    :func:`encode_signed_consistency` over ``receipt.consistency``. No new
    signing message or signing domain is introduced: encoding is read-only
    and only re-uses the existing canonical encodings, so a continuation
    credential can be written to disk and restored in another process.
    ...
    proof or bundle↔consistency linkage do not match encodes just as well.
    """
    if not isinstance(receipt, SignedAuthAuditContinuation):
        raise TypeError(
...
def decode_signed_auth_audit_continuation(
    ...
    ``data`` must be ``bytes`` (anything else, including ``bytearray`` and
    ``memoryview``, raises TypeError). After the magic
    ``b"auditchain/auth-audit-continuation/v1\\0"`` it must contain, strictly
    in order, the u64 envelope version (only ``1`` is supported), one
    length-prefixed bundle blob and one length-prefixed consistency blob,
    with no trailing bytes. The two blobs are handed whole to the existing
    decoders — :func:`decode_signed_auth_audit_bundle` and
    :func:`decode_signed_consistency` respectively — so every nested magic,
    version, truncation/trailing-byte and structural rule is theirs,
    including the auth-audit decoder's hash-algorithm agreement check. A bad
    magic or version, truncation, an oversized blob length, trailing bytes
    or an illegal nested encoding raises ValueError.

    Signatures, tags, the consistency proof and the bundle↔consistency
    linkage are not checked here. The returned object is a frozen
    :class:`SignedAuthAuditContinuation`, positionally constructed with the
    field order preserved, whose fields equal the originally encoded ones,
    and re-encoding reproduces the original bytes exactly. A structurally
    sound encoding whose signatures simply do not verify, or whose
    consistency's ``new`` checkpoint differs from the audit checkpoint,
    ...
    if offset != len(data):
        raise ValueError(
            "trailing bytes after the auth-audit continuation"
        )
    # Decode both nested blobs with their existing decoders; their own
    # magic, version, truncation/trailing-byte and structural checks apply
    # verbatim (the bundle decoder also pins its two hash algorithms).
    bundle = decode_signed_auth_audit_bundle(bundle_blob)
    consistency = decode_signed_consistency(consistency_blob)
    return SignedAuthAuditContinuation(bundle, consistency)
