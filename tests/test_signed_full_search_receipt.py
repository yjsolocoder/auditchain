...
class SignedFullSearchReceiptIssueTest(unittest.TestCase):
    ...
    def test_explicit_range_and_size(self):
        ...
        self.assertEqual(bundle.receipt, self.log.full_search_receipt("hit", 1, 4, 4))
        ...
        self.assertTrue(verify_signed_full_search_receipt(bundle, self.public_key))
    ...
    def test_empty_range(self):
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A, 2, 2)
        self.assertEqual(bundle.receipt.items, ())
        self.assertEqual(bundle.receipt.proof, ())
        self.assertTrue(verify_signed_full_search_receipt(bundle, self.public_key))

    def test_no_hits_still_covers_full_range(self):
        bundle = self.log.signed_full_search_receipt("absent", _SEED_A)
        self.assertEqual(len(bundle.receipt.items), len(self.log))
        self.assertTrue(verify_signed_full_search_receipt(bundle, self.public_key))
    ...
    def test_constructor_rejects_parts_of_different_snapshots(self):
        ...
        other_receipt = self.log.full_search_receipt("hit", size=4)
        with self.assertRaises(ValueError):
    ...
    def test_failure_leaves_state_unchanged(self):
        ...
        with self.assertRaises(TypeError):
            self.log.signed_full_search_receipt(bytearray(b"hit"), _SEED_A)
        with self.assertRaises(TypeError):
...
class VerifySignedFullSearchReceiptTest(unittest.TestCase):
    ...
    def test_wrong_public_key_returns_false(self):
        self.assertFalse(
            verify_signed_full_search_receipt(self.bundle, self.other_public_key)
        )
    ...
    def test_other_key_signing_returns_false(self):
        ...
        self.assertTrue(
            verify_signed_full_search_receipt(foreign, self.other_public_key)
        )
    ...
    def test_public_key_validation(self):
        ...
        with self.assertRaises(ValueError):
            verify_signed_full_search_receipt(self.bundle, self.public_key[:-1])
        with self.assertRaises(ValueError):
            verify_signed_full_search_receipt(
