...
# Binary framing of dump_signed_pruned_hybrid / load_signed_pruned_hybrid:
# the Ed25519-signed counterpart of the pruned-hybrid framing — a pruned,
# keyed, forward-secure log whose history may contain encrypted entries
# (including ciphertexts released by the prune), exported as one
...
# B(head) || U(stage) || B(K) || U(x), with F the pruned-log frontier, Q the
# complete lifetime nonce history and each Ei a secure-log record with
# B(locator)) and S the 64-byte Ed25519 signature over every preceding byte;
# no further signing domain is introduced and no key is encrypted. As with
...
def dump_signed_pruned_hybrid(log: Any, private_key: Any) -> bytes:
    ...
    was **constructed with an authentication key** qualifies; its history may
    contain entries appended with :meth:`AuditLog.encrypt`, including
    ciphertexts released by the prune. The framing carries the prune
    checkpoint, the prefix frontier, the retained entries, the log's complete
    nonce history (covering every encrypt nonce ever used, even those of
    released ciphertexts), the encrypted-locator HMACs of the retained
    ciphertexts and the current forward-secure evolution key and stage in the
    clear, so a fresh process can continue evolving from exactly the same
    point, but it carries no prune receipts, encryption keys or verifier
    material beyond the u64 verifier-exported flag.
    ...
    **not** encrypt the framing, which carries the live evolution key, so the
    ...
    # and per-entry locators, so only a pruned log that was constructed with
    # a key qualifies; encrypt history — including ciphertexts released by
    # the prune — is exactly what the nonce/locator fields preserve.
    if log._retain_from == 0:
        raise ValueError(
    ...
    root = log._fold_occupied(log._occupied_at(size))
    head = log._chain_head_at(size)
    # Field-by-field the plaintext framing of dump_pruned_hybrid.
    parts = [
...
def load_signed_pruned_hybrid(data: Any, public_key: Any) -> AuditLog:
    ...
    Verification happens before any state is built: the trailing signature is
    checked against every preceding byte with ``public_key``; only then is
    the stream parsed and re-checked: ``r`` must satisfy ``0 < r <= n``, the
    checkpoint must have the digest width, ``F`` must list one
    ``U(height) || B(digest)`` pair per set bit of ``r`` in strictly ascending
    order, ``Q`` must carry the complete lifetime nonce history (every blob
    exactly 12 bytes, the nonces distinct and in lexicographic order), ``E``
    must carry exactly ``n - r`` records at indices ``r..n-1`` in order, each
    in the :func:`dump_secure_log` field order (``U, B, B, B, B``) whose
    ...
    that is not 32 bytes, a bad magic, version, UTF-8 or hash algorithm,
    truncation, trailing bytes, an out-of-range retain point, a wrong
    checkpoint/frontier/digest/locator width, a frontier that is not exactly
    the set bits of ``r``, a wrong-width, duplicated or misordered history
    nonce, an entry count that disagrees with ``n - r``, an out-of-order
    index, an unparseable ciphertext envelope, a duplicate retained
    ciphertext nonce, a retained ciphertext nonce missing from the history,
    an out-of-range stage or flag, a wrong-width evolution key, a signature
    that does not verify or a recomputed chain-head/root mismatch raises
    ...
    # hash algorithm, and simultaneously replay the payloads into a fresh
    # keyed log with the checkpoint, frontier and retain point installed, so
    # every auxiliary structure (find index, encrypted locator index, nonce
    # history, frontier) is rebuilt exactly as the normal append / encrypt
    # paths build it. The complete nonce history is seeded before the replay
    # (covering nonces of ciphertexts released by the prune); the evolution
    # key, stage and verifier-exported flag are only installed after the
