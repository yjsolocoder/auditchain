...
# encode_signed_audit_batch bytes, in that order and with nothing else. As in
# the auth-audit framing the two nested packages do not name a common
# algorithm themselves: the decoder additionally requires the stage auth
# bundle's hash algorithm to equal the signed audit batch's algorithm.
...
def encode_signed_stage_auth_audit_bundle(x: Any) -> bytes:
    """Encode a :class:`SignedStageAuthAuditBundle` into canonical bytes.

    The byte stream is ``D || U(1) || B(A) || B(M)`` with
    ``D = b"auditchain/signed-stage-auth-audit/v1\\0"``, ``U`` an unsigned
    8-byte big-endian integer and ``B(x) = U(len(x)) || x``: the envelope
    ``version`` (always 1), then the stage auth blob and the audit blob —
    nothing may be omitted, reordered or appended. ``A`` is the complete
    canonical output of :func:`encode_signed_stage_auth_bundle` over
    ``x.auth`` and ``M`` the complete canonical output of
    :func:`encode_signed_audit_batch` over ``x.audit``. No new signing
    message is introduced: encoding is read-only and only re-uses the
    existing canonical encodings.

    ``x`` must be a :class:`SignedStageAuthAuditBundle` — anything else
    raises TypeError; nested structural problems raise exactly the
    exceptions of :func:`encode_signed_stage_auth_bundle` and
    :func:`encode_signed_audit_batch` (TypeError or ValueError), and a
    bundle whose signed audit batch names a different hash algorithm than
    the signed stage auth bundle raises ValueError — encoding never leaves
    a half result. The encoding carries the stage key in the clear — it
    authenticates provenance only, never encrypts — so the bytes must be
    protected exactly like a bare :class:`StageVerifier`. Encoding is
    deterministic: re-encoding a decoded bundle reproduces the original
    bytes exactly, and a structurally valid bundle whose signatures, tags,
    proof or checkpoint do not match encodes just as well.
    """
    if not isinstance(x, SignedStageAuthAuditBundle):
        raise TypeError("x must be a SignedStageAuthAuditBundle")
    # Re-validate the container even for an instance whose fields were set
    # bypassing the frozen constructor, so container-type corruption raises
    # TypeError exactly as the constructor would.
    checked = SignedStageAuthAuditBundle(x.auth, x.audit)
    # Encode both nested parts first; their own re-validation classifies
    # every nested type/structure error with its existing TypeError /
    # ValueError discipline. Only once both canonical byte strings exist
    # do we require them to describe one delivery: the audit batch must be
    # minted under the hash algorithm the signed stage auth bundle names
    # (its own encoder has already pinned that to the signed stage
    # verifier's algorithm). A disagreeing bundle raises ValueError
    # without returning any (half) result.
...
def decode_signed_stage_auth_audit_bundle(
    ...
    strictly in order, the u64 envelope version (only ``1`` is supported),
    one length-prefixed stage auth blob and one length-prefixed audit blob,
    with no trailing bytes. Each blob is handed whole to the existing
    decoder — :func:`decode_signed_stage_auth_bundle` and
    :func:`decode_signed_audit_batch` respectively — so every nested
    framing and structural rule is theirs (the stage auth decoder also
    pins its batch algorithm to the signed stage verifier's), and the
    stage auth bundle's hash algorithm must equal the audit batch's
    algorithm. A bad magic or version, truncation, an oversized blob
    length, trailing bytes, an illegal nested encoding or a hash algorithm
    disagreement between the two packages raises ValueError.

    The returned object is a frozen :class:`SignedStageAuthAuditBundle`
    whose two fields equal the originally encoded ones (an empty selection
    round-trips as ``()`` and the empty snapshot omits no structure), and
    re-encoding reproduces the original bytes exactly. A structurally
    sound encoding whose signatures, tags, proof or checkpoint simply do
    not verify still decodes;
    :func:`verify_signed_stage_auth_audit_bundle` reports False.
    """
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
    ...
    # Decode both nested blobs with their existing decoders; their own
    # magic, version, truncation/trailing-byte and structural checks apply
    # verbatim (the stage auth decoder also pins its verifier algorithm).
