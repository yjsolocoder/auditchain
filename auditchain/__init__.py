...
# decode_encrypted_search_receipt: same u64/blob rules; a content-search
# receipt over encrypted entries, binding the normalized plaintext query, the
# searched half-open range and the snapshot context to one inclusion proof
# per hit. Verification additionally needs the append-time encryption key,
# which the receipt itself never carries.
_ENCRYPTED_SEARCH_RECEIPT_MAGIC = b"auditchain/encrypted-search/v1\0"
_ENCRYPTED_SEARCH_RECEIPT_VERSION = 1
...
class EncryptedSearchReceipt:
    """Offline receipt for a plaintext search over encrypted entries.

    Issued by :meth:`AuditLog.encrypted_search_receipt` and verified entirely
    offline by :func:`verify_encrypted_search_receipt` (which additionally
    needs the append-time encryption key; the receipt itself never carries
    it):
    ...
    - ``size``: number of entries in the snapshot the receipt refers to,
    - ``root``: Merkle root of that snapshot,
    - ``query``: the normalized plaintext query value (a ``str`` query is
      UTF-8 encoded at construction; the stored value is always ``bytes``),
    - ``start`` / ``stop``: the half-open absolute-index range the search
      covered, satisfying ``0 <= start <= stop <= size``,
    - ``items``: ``(Entry, proof)`` pairs in strictly ascending absolute
      index order, one per listed hit, where ``proof`` is the tuple of
      sibling digests of the entry's inclusion proof within the snapshot.
      Every listed entry must satisfy ``start <= entry.index < stop`` and
      carries the encrypted-entry envelope as its payload. Duplicate
      plaintexts are listed once per occurrence; a search with no hits (and
      any empty snapshot) carries ``items == ()``.
...
class AuditLog:
    ...
    def encrypted_search_receipt(
        ...
        """Issue an offline :class:`EncryptedSearchReceipt` for a plaintext
        search over encrypted entries.
        ...
        log: the receipt records the hash algorithm, the snapshot ``size``
        and its Merkle ``root``, the normalized plaintext ``query``, the
        searched range and one ``(Entry, inclusion_proof)`` pair per hit in
        ascending absolute index order, duplicates listed once per
        occurrence. Each listed entry carries its encrypted-entry envelope as
        the payload; the key itself is never recorded.

        ``query`` accepts ``bytes`` or ``str`` (UTF-8 encoded); anything else
        raises TypeError. ``key`` must be exactly 32 ``bytes`` — a non-bytes
        value raises TypeError and a wrong length raises ValueError. ``size``
        ...
        ValueError. Plain entries and entries sealed under other keys never
        hit, so a valid but wrong key simply yields ``items == ()``. A search
        with no hits — and any empty snapshot — yields ``items == ()``. The
        call is read-only: entries, head, authentication state, Merkle roots
        ...
        return EncryptedSearchReceipt(
            version=_ENCRYPTED_SEARCH_RECEIPT_VERSION,
            hash_name=self._hash_name,
            size=size,
            root=root,
            query=material,
...
def verify_encrypted_search_receipt(receipt: Any, key: Any) -> bool:
    ...
    Recomputes every listed entry's digest from the entry's fields,
    re-verifies every inclusion proof against the receipt's snapshot root,
    then decrypts every listed envelope with ``key`` and compares the
    recovered plaintext against the normalized query value; a structurally
    valid receipt whose entry content, proofs, root or decrypted plaintexts
    do not match returns False. Verification only attests that the listed
    hits are genuine — it makes no claim about whether the result set is
    complete, so a receipt listing fewer hits than the log would have found
    is not a failure. A valid 32-byte key that simply was not the append key
    (or content that does not decrypt to the query) yields False rather than
    raising. ``key`` must be exactly 32 ``bytes`` — a non-bytes value raises
    TypeError and a wrong length raises ValueError. Malformed input raises
    TypeError or ValueError exactly as :class:`EncryptedSearchReceipt`
    construction does (a non-:class:`EncryptedSearchReceipt` argument, or a
    receipt whose frozen fields were bypassed into an illegal shape, raises
    the same errors). The call is read-only.
    """
    ...
    for entry, proof in checked.items:
        ...
        try:
            ...
            # A listed envelope that does not authenticate under this key (a
            # wrong key, a corrupted or plain payload) is not a genuine hit.
            return False
        if plaintext != checked.query:
            return False
    return True
...
def _check_encrypted_search_receipt_proofs(receipt: EncryptedSearchReceipt) -> None:
    for entry, proof in receipt.items:
        expected = _proof_level_count(entry.index, receipt.size)
        if len(proof) != expected:
            raise ValueError(
...
def encode_encrypted_search_receipt(receipt: Any) -> bytes:
    ...
    ``previous_hash`` blob, ``entry_hash`` blob, proof count and one blob
    per proof digest, with nothing omitted, reordered or appended.
    ``receipt`` must be an :class:`EncryptedSearchReceipt` (anything else
    ...
    an illegal shape raises the same TypeError or ValueError, and a proof
    whose level count does not fit its ``(index, size)`` raises ValueError.
    ...
    _check_encrypted_search_receipt_proofs(checked)
    parts = [
        _ENCRYPTED_SEARCH_RECEIPT_MAGIC,
        _encode_u64(checked.version, "version"),
        _encode_blob(checked.hash_name.encode("utf-8")),
        _encode_u64(checked.size, "size"),
        _encode_blob(bytes(checked.root)),
...
def decode_encrypted_search_receipt(data: Any) -> EncryptedSearchReceipt:
    ...
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
    if not data.startswith(_ENCRYPTED_SEARCH_RECEIPT_MAGIC):
        raise ValueError("not an auditchain encrypted-search-receipt encoding")
    offset = len(_ENCRYPTED_SEARCH_RECEIPT_MAGIC)
    ...
    _check_encrypted_search_receipt_proofs(receipt)
    return receipt
