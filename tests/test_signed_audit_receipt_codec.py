...
class EncodeSignedAuditReceiptTest(unittest.TestCase):
    ...
    def test_magic_and_field_layout(self):
        ...
        offset += 8
        # receipt blob: u64 length then the complete encode_audit_receipt
        # bytes: B(R).
    ...
    def test_nested_receipt_value_error_propagates(self):
        ...
        # index/size is rejected by encode_audit_receipt's ValueError.
        receipt = bundle.receipt
        entry, proof = receipt.items[0]
        forged_receipt = AuditReceipt.__new__(AuditReceipt)
        for name in ("version", "hash_name", "size", "root"):
            object.__setattr__(
...
class DecodeSignedAuditReceiptTest(unittest.TestCase):
    ...
    def roundtrip(self, bundle):
        ...
        self.assertIs(type(decoded.receipt), AuditReceipt)
    ...
    def test_nested_snapshot_mismatch_rejected(self):
        ...
        framed = (
            MAGIC
            + u64(1)
            + blob(encode_audit_receipt(forged.receipt))
            + blob(encode_signed_root(forged.checkpoint))
        )
        with self.assertRaises(ValueError):
            decode_signed_audit_receipt(framed)
    ...
    def test_call_is_read_only(self):
        ...
        self.assertEqual(
            decode_signed_audit_receipt(data), bundle
        )
