...
`search_receipt` 只判定列出的命中属实，不判定结果集是否完整；`full_search_receipt`
补上这一层：回执携带被搜范围内**每一个**条目（而非仅命中），并附一份覆盖全部
条目的共享紧凑批量证明。核验方无需持有日志，先认证条目真伪，再自行比对查询值，
得出的就是范围内全部命中——少列条目在构造与核验两端都是结构错误，无法隐瞒：

```python
receipt = log.full_search_receipt("agent started")  # 范围默认整个保留段，size 默认当前长度
receipt.query                                       # b"agent started"：str 按 UTF-8 规范化
[e.index for e in receipt.items]                    # [0, 1, 2, ...]：范围内逐条列出
[e.index for e in receipt.items if e.payload == receipt.query]  # 核验方自行得出的完整命中集
verify_full_search_receipt(receipt)                 # True：无需持有日志即可离线核验
log.full_search_receipt("agent", 1, 3, size=3)      # 半开范围 [1, 3) + 前 3 条快照
```

回执为冻结的 `FullSearchReceipt(version=1, hash_name, size, root, query, start,
stop, items, proof)`，支持位置构造、按全部九个字段相等：记录算法、快照尺寸与
Merkle 根、规范化查询值、半开范围 `[start, stop)`、该范围内**每一个** `Entry`
（按绝对索引严格升序，恰为 `start .. stop-1` 逐条各一份）以及一份覆盖它们的共享
紧凑批量证明（与 `batch_inclusion_proof` 同一布局，不再逐条附包含证明）。查询值
只收 `bytes` 或 `str`（后者按 UTF-8 规范化）；其余二进制字段——`root`、每个条目的
`payload` / `previous_hash` / `entry_hash` 与每个共享证明节点——只接受精确的
`bytes`，`bytearray` 与 `memoryview` 抛 `TypeError`，收到的制品不会悄悄别名调用方
的可变缓冲。范围默认 `[retain_from, size)`，`size` 默认当前长度且快照须可重建。
空范围与空快照照样可以签发和核验，此时条目与证明都是空元组。签发为只读、可重复
调用，不改变条目、`head`、认证状态、Merkle 根或证明；查询值、范围或尺寸类型非法
抛 `TypeError`，范围或尺寸越界、快照不可重建抛 `ValueError`。

离线核验逐条重算 `entry_digest`，再用共享批量证明（`verify_batch_inclusion`）
核对快照根；由于范围内条目无一缺失，核验方自行比对 `entry.payload == receipt.query`
即得完整命中集。条目覆盖不全、重复或乱序在构造端即抛 `ValueError`（绕过冻结构造器
写入的非法字段在核验端同样复验）；结构合法但条目内容、证明或根不符返回 `False`；
入参不是 `FullSearchReceipt` 抛 `TypeError`，证明节点数与所列索引及 `size` 不符抛
`ValueError`。

回执可编码为规范字节形式，落盘或传输后恢复并继续离线核验：

```python
data = encode_full_search_receipt(receipt)      # bytes：魔数 + u64 大端整数 + 长度前缀 blob
restored = decode_full_search_receipt(data)     # 字段与原回执相等
encode_full_search_receipt(restored) == data    # True：重复编码字节相同
verify_full_search_receipt(restored)            # True
```

编码以魔数 `b"auditchain/full-search/v1\0"`（含 NUL 结尾）开头，后接恒为 1 的
`version`，随后严格按回执字段顺序写 `hash_name`（UTF-8 blob）、`size`、`root`
blob、`query` blob、`start`、`stop` 与条目计数；每个条目依次写 `Entry.index`、
`payload` blob、`previous_hash` blob、`entry_hash` blob（与 `encode_audit_receipt`
的条目写法相同），最后写共享证明的节点计数及各摘要 blob；整数均为 8 字节无符号
大端，blob 为 u64 字节长度后接原始字节（零长度也是全零 u64），禁止换序与尾随。
`encode_full_search_receipt` 只接受 `FullSearchReceipt` 并复验全部字段与证明节点
计数（类型错抛 `TypeError`，结构非法抛 `ValueError`）；
`decode_full_search_receipt` 只接受精确的 `bytes`（拒绝 `bytearray` /
`memoryview`），魔数、版本、非法 UTF-8、未知算法、截断、尾随、长度越界、范围、
顺序或覆盖非法、摘要宽度或证明节点数不符均抛 `ValueError`。空范围与空快照照常
往返且核验通过；内容被篡改的回执仍可往返，仅核验判 `False`。本层不新增签名
原文，签名封装见下一小节，旧接口与各线格式行为不变。

#### 完整检索回执的可信签名封装（Ed25519）

`signed_full_search_receipt` 在一个不可变 `SignedFullSearchReceipt` 中同时打包
`full_search_receipt` 的全覆盖 `FullSearchReceipt` 与 `sign_root` 对同一快照签发的
`SignedRoot` 检查点；离线接收方仅凭**预置信任**的 32 字节 Ed25519 公钥，即可一次
确认被搜范围内每一个条目与共享批量证明、快照 Merkle 根与链头均由日志持有者签发，
无需持有 `AuditLog`，也不引入任何新的签名原文：

```python
from auditchain import verify_signed_full_search_receipt

bundle = log.signed_full_search_receipt("agent started", seed)  # 范围默认整个保留段
bundle.receipt == log.full_search_receipt("agent started")      # True：完整检索回执
...
log.signed_full_search_receipt("agent", seed, 1, 3, size=3)     # 半开范围 [1, 3) + 前 3 条快照
```

- 包为冻结的 `SignedFullSearchReceipt(receipt:FullSearchReceipt,
  checkpoint:SignedRoot)`，字段顺序固定，支持位置构造、按两个字段相等；`receipt`
  必须是 `FullSearchReceipt`、`checkpoint` 必须是 `SignedRoot`，容器字段类型错抛
  `TypeError`；两部分必须描述**同一可重建快照**（`hash_name`、`size`、`root`
  相等），否则在构造与编解码两端都抛 `ValueError`；回执自身的结构契约由
  `FullSearchReceipt` 校验，签名真伪留给验包
- `AuditLog.signed_full_search_receipt(query, private_key, start=None, stop=None,
  size=None)` 语义沿用完整检索签发：`query` 只收 `bytes` 或 `str`（后者按 UTF-8
  规范化），范围默认 `[retain_from, size)`、`size` 默认当前长度且快照须可重建；
  先调用 `full_search_receipt(query, start, stop, size)`，再调用
  `sign_root(private_key, size)`，据其结果构造 `SignedFullSearchReceipt`。签发
  只读、可重复：同状态同种子两次签发逐字节相同；任何失败都在构造前抛出，不留
  半成品、不改日志，也不新增签名原文（检查点签的仍是 `sign_root` 的原文）
- `verify_signed_full_search_receipt(bundle, public_key)` 完全离线核验，三项皆真
  才返回 `True`：`verify_full_search_receipt` 为真（逐条重算摘要并用共享批量证明
  核对快照根）、`verify_signed_root` 为真（检查点签名与预置公钥相符），且两部分
  的算法（`hash_name`）、`size`、`root` 一致。公钥不受信任、两部分不一致，或条目
  / 证明 / 根 / 签名被改，均返回 `False`（绝不抛异常）
- 查询值、范围、`size`、种子、公钥及各入参的类型问题抛 `TypeError`；种子或公钥
  非 `bytes` 抛 `TypeError`、非 32 字节抛 `ValueError`；取值或宽度非法、范围
  越界、快照不可重建、嵌套结构非法抛 `ValueError`；验包为只读
- `encode_signed_full_search_receipt(bundle)` /
  `decode_signed_full_search_receipt(data)` 把整个可信完整检索回执序列化为规范
  二进制并原样还原，使其可落盘、跨进程传输后继续凭预置信任的 Ed25519 公钥离线
  验真，且不引入任何新的签名原文：
...
encode_signed_full_search_receipt(restored) == data       # True：重复/解码后重编码逐字节相同
verify_signed_full_search_receipt(restored, public_key)   # True：无需持有日志
```

  字节流以魔数 `b"auditchain/signed-full-search/v1\0"`（含 NUL 结尾）开头，随后
  **严格依次**写 `version`（恒为 `1`，8 字节无符号大端，即 `U(1)`）、receipt
  blob（`B(R)`）、checkpoint blob（`B(C)`），顺序固定、禁止换序与尾随；两个 blob
  均为 u64 字节长度前缀加原始字节，内容依次就是既有 `encode_full_search_receipt`
  与 `encode_signed_root` 输出的完整规范字节，`U` / `B` 规则与既有框架完全一致。
  解码精确消费两个 blob 且禁止尾随，分别原样交给 `decode_full_search_receipt` 与
  `decode_signed_root`。`encode_signed_full_search_receipt` 只接受
  `SignedFullSearchReceipt`（其余类型抛 `TypeError`，两部分快照不一致或嵌套错误
  抛 `ValueError`），`decode_signed_full_search_receipt` 只接受精确的 `bytes`
  （含拒绝 `bytearray` / `memoryview`，抛 `TypeError`）；魔数、版本、截断、尾随、
  blob 长度越界、两部分快照不一致或任一嵌套格式非法抛 `ValueError`；结构合法但
  验真不匹配仍可解码，`verify_signed_full_search_receipt` 返回 `False`。空范围与
...
`encrypted_search_receipt` 把一次 `find_encrypted` 的结果固化成可落盘、可离线
核验的检索回执：无需持有日志，`verify_encrypted_search_receipt` 持查询密钥即可
逐条判定列出的命中是否属实。回执只记录密封的密文封装，**既不保存密钥也不保存
明文**：

```python
receipt = log.encrypted_search_receipt("secret event", key)  # 范围默认保留段，size 默认当前长度
receipt.query                                                # b"secret event"：str 按 UTF-8 规范化
[e.index for e, _ in receipt.items]                          # [0]：命中条目仍带密封封装
verify_encrypted_search_receipt(receipt, key)                # True：离线解密比对
verify_encrypted_search_receipt(receipt, wrong_key)          # False：密钥错误不抛异常
log.encrypted_search_receipt("secret event", key, 1, 3, size=3)  # 半开范围 [1, 3) + 前 3 条快照
```

回执为冻结的
`EncryptedSearchReceipt(version=1, hash_name, size, root, query, start, stop,
items)`，支持位置构造、按全部字段相等：记录算法、快照尺寸与 Merkle 根、规范化
查询值、半开范围 `[start, stop)` 与命中条目；每个命中是 `(Entry, 包含证明)`
对，条目的 `payload` 仍是密封的加密封装，按绝对索引严格升序、重复明文逐次列出，
无命中（及空快照）时 `items == ()`。查询值只收 `bytes` 或 `str`（后者按
UTF-8 规范化），其他类型抛 `TypeError`；密钥只收 32 字节 `bytes`（类型错抛
`TypeError`，长度不符抛 `ValueError`）；范围默认 `[retain_from, size)`，
`size` 默认当前长度且快照须可重建，范围/尺寸类型错抛 `TypeError`、越界或快照
不可重建抛 `ValueError`。异钥追加的条目与普通条目不命中；签发为只读，不改变
条目、`head`、认证状态、密文定位索引、Merkle 根或证明。

离线核验逐条重算 `entry_digest`、核验包含证明落在记录的快照根内，再用
`decrypt_entry` 以传入密钥解密密封封装并把恢复出的明文与规范化查询值逐字节
比较；**核验只判定列出的命中是否属实，不判定结果集是否完整**，少列条目不算
失败。密钥错误、AEAD 认证失败、列出的是普通条目或明文不符都返回 `False` 而不
抛异常；入参不是 `EncryptedSearchReceipt` 或密钥类型错抛 `TypeError`，密钥长度、
范围与尺寸越界、快照不可重建、未知算法或摘要宽度不符（含绕过冻结构造器写入的
非法字段）抛 `ValueError`。

回执可编码为规范字节形式，落盘或传输后恢复并继续离线核验：

```python
data = encode_encrypted_search_receipt(receipt)      # bytes：魔数 + u64 大端整数 + 长度前缀 blob
restored = decode_encrypted_search_receipt(data)     # 恢复对象冻结、按字段相等
encode_encrypted_search_receipt(restored) == data    # True：重复/解码后重编码逐字节相同
verify_encrypted_search_receipt(restored, key)       # True
```

编码以魔数 `b"auditchain/encrypted-search/v1\0"`（含 NUL 结尾）开头，后接恒为
1 的 `version`，随后严格按回执字段顺序写 `hash_name`（UTF-8 blob）、`size`、
`root` blob、`query` blob、`start`、`stop` 与命中计数；除魔数外布局与
`encode_search_receipt` 完全一致——每个命中依次写 `Entry.index`、`payload`
blob（密封封装原样写入）、`previous_hash` blob、`entry_hash` blob、证明计数及
各摘要 blob，整数均为 8 字节无符号大端，blob 为 u64 字节长度后接原始字节
（零长度也是全零 u64），禁止换序与尾随。`encode_encrypted_search_receipt` 只
接受 `EncryptedSearchReceipt` 并复验全部字段与证明层数（类型错抛 `TypeError`，
结构非法或整数溢出 u64 抛 `ValueError`）；`decode_encrypted_search_receipt` 只
接受精确的 `bytes`（拒绝 `bytearray` / `memoryview`），魔数或版本不符、非法
UTF-8、未知算法、截断、尾随、长度越界、范围或顺序非法、摘要宽度或证明层数不符
均抛 `ValueError`。解码精确消费全部字节，空结果与空快照照常往返；**内容被篡改
的回执照常往返、仅核验判 `False`**；本层不新增任何签名原文，也不改变其他各线
格式。

#### 按原明文定位的完整加密检索回执（全覆盖）

`encrypted_search_receipt` 只判定列出的命中属实，不判定结果集是否完整；
`full_encrypted_search_receipt` 补上这一层：回执携带被搜范围内**每一个**条目
（`payload` 仍是密封封装），附一份覆盖全部条目的共享紧凑批量证明，并记录签发方
当场用查询密钥逐条解封比对得出的命中索引。持钥核验方无需持有日志，先认证条目
真伪，再自行逐条解封比对，重算出的命中集与记录逐项一致才判真——范围内任何命中
都无法隐瞒或伪造：

```python
receipt = log.encrypted_search_receipt("secret event", key)          # 只判定列出命中属实
receipt = log.full_encrypted_search_receipt("secret event", key)     # 范围默认保留段，size 默认当前长度
receipt.query                                                        # b"secret event"：str 按 UTF-8 规范化
[e.index for e in receipt.items]                                     # [0, 1, 2, ...]：范围内逐条列出
receipt.hits                                                         # (0, ...)：签发方当场解封得出的命中索引
verify_full_encrypted_search_receipt(receipt, key)                   # True：离线重算命中集并逐项核对
verify_full_encrypted_search_receipt(receipt, wrong_key)             # False：记录了命中时错误密钥必判假
log.full_encrypted_search_receipt("secret event", key, 1, 3, size=3) # 半开范围 [1, 3) + 前 3 条快照
```

回执为冻结的 `FullEncryptedSearchReceipt(version=1, hash_name, size, root, query,
start, stop, items, proof, hits)`，支持位置构造、按全部十个字段相等：记录算法、
快照尺寸与 Merkle 根、规范化查询值、半开范围 `[start, stop)`、该范围内**每一个**
`Entry`（按绝对索引严格升序，恰为 `start .. stop-1` 逐条各一份，`payload` 仍是
密封的加密封装）、一份覆盖全部条目的共享紧凑批量证明（与 `batch_inclusion_proof`
同一布局），以及按升序记录的命中索引元组 `hits`；回执**既不保存密钥也不保存
明文**。查询值只收 `bytes` 或 `str`（后者按 UTF-8 规范化），其他类型抛
`TypeError`；密钥只收 32 字节 `bytes`（类型错抛 `TypeError`，长度不符抛
`ValueError`）；其余二进制字段——`root`、每个条目的 `payload` / `previous_hash`
/ `entry_hash` 与每个共享证明节点——只接受精确的 `bytes`，`bytearray` 与
`memoryview` 抛 `TypeError`。范围默认 `[retain_from, size)`，`size` 默认当前长度
且快照须可重建，范围/尺寸类型错抛 `TypeError`、越界或快照不可重建抛
`ValueError`。普通条目、异钥封装与解密失败都不算命中也不判失败；空范围与空快照
照常签发核验，此时条目、证明与命中都是空元组。签发为只读、可重复调用，不改变
条目、`head`、认证状态、密文定位索引、Merkle 根或证明。

离线核验逐条重算 `entry_digest`、用共享批量证明（`verify_batch_inclusion`）核对
快照根（空快照只认规范空树根），再以传入密钥逐条尝试解封，成功且明文等于规范化
查询值者计为命中；**重算出的命中集与回执记录的 `hits` 逐项一致且真实性成立才
返回 `True`**。条目覆盖不全、重复、乱序、越出范围或命中索引不合法（非严格升序、
越出范围）在构造与核验两端都抛 `ValueError`（绕过冻结构造器写入的非法字段在
核验端同样复验）；篡改的内容、证明或根一律返回 `False` 不抛异常；记录了命中的
回执在错误密钥下返回 `False`，零命中回执不考验密钥、只按真实性判定。入参不是
`FullEncryptedSearchReceipt` 或密钥类型错抛 `TypeError`，密钥长度、范围与尺寸
越界、未知算法、摘要宽度或证明节点数不符抛 `ValueError`。本层暂不提供规范二进制
编解码，也不新增签名原文；既有查找、加密检索回执与签名行为不变。
...
可跨进程恢复的冻结制品：`AnchorSet(items:tuple)`。唯一字段是按链序存放的
`AnchoredContinuationChain` 包**非空 tuple**，不再携带别的字段；支持
位置/关键字构造、按该字段相等（可哈希）。制品本身只校验容器形状：`items`
须为非空 tuple 且元素均为 `AnchoredContinuationChain`——非 tuple 或元素
类型错抛 `TypeError`，空集抛 `ValueError`；各包是否验真、包间锚点接缝是否
成立不在此处校验，恢复后可在另一进程中把 `items` 直接交给既有的
`inspect_anchor_set` / `merge_anchor_set` 继续诊断或合并，结论与原件一致。
...
- 字节流严格为 `D || U(1) || U(n) || B(P0)…B(Pn-1)`，其中
  ...
  `encode_anchored_continuations(items[i])` 的完整输出，按 tuple 顺序
  排列、**禁止省略、换序与尾随字节**；全部 blob 复用既有编码，外层帧
  不引入新签名域
...
- `decode_anchor_set(data) -> AnchorSet` 只接受 `bytes`（拒绝
  `bytearray` / `memoryview`，抛 `TypeError`）；魔数 / 版本错、空集
  （包数为零）、截断、blob 长度越界或尾随字节抛 `ValueError`；各包
  blob 交给 `decode_anchored_continuations`，嵌套异常原样传播。解码
  保序并返回冻结对象，但不校验签名、证明、包间接缝与锚定关系，结构
  合法而验真不匹配仍可解码，由 `inspect_anchor_set` 报告而非抛出
...
- `FullSearchReceipt(version, hash_name, size, root, query, start, stop, items,
  proof)` — 不可变的完整检索回执，把一次按内容查找的**全覆盖**结果（算法、
  快照尺寸与 Merkle 根、规范化查询值、半开范围 `[start, stop)`、该范围内每一个
  `Entry` 与一份共享紧凑批量证明）绑成一件可落盘制品，按全部九个字段相等、支持
  位置构造；`version` 恒为 `1`，`query` 只收 `bytes` 或 `str`（后者按 UTF-8
  规范化后存储），范围满足 `0 <= start <= stop <= size`，`items` 按绝对索引严格
  升序且恰为 `start .. stop-1` 逐条各一份（覆盖不全、重复或乱序均非法），
  `proof` 为覆盖全部所列条目的共享紧凑批量证明（与 `batch_inclusion_proof`
  同一布局，空范围时与 `items` 同为 `()`）；核验方认证条目后自行比对查询值即得
  完整命中集。`query` 以外的二进制字段（`root`、每个条目的 `payload` /
  `previous_hash` / `entry_hash` 与每个证明节点）只接受精确的 `bytes`，
  `bytearray` / `memoryview` 抛 `TypeError`。类型非法抛 `TypeError`，版本、范围、
  未知算法、摘要宽度、条目结构或覆盖非法（含空范围携带非空证明）抛 `ValueError`；
  规范二进制编解码由 `encode_full_search_receipt` /
  `decode_full_search_receipt` 提供
- `EncryptedSearchReceipt(version, hash_name, size, root, query, start, stop,
  items)` — 不可变的离线加密检索回执，把一次 `find_encrypted` 的结果（算法、
  快照尺寸与 Merkle 根、规范化查询值、半开范围 `[start, stop)` 与命中条目）绑成
  一件可落盘制品，按全部八个字段相等、支持位置构造；字段形状契约与
  `SearchReceipt` 相同（`version` 恒为 `1`，`query` 只收 `bytes` 或 `str`，
  范围满足 `0 <= start <= stop <= size`，`items` 按绝对索引严格升序且每个命中
  索引落在记录范围内，重复明文逐次列出，无命中与空快照为 `()`），区别仅在于
  命中条目的 `payload` 是仍密封的加密封装、回执不记录密钥或明文，其是否确实
  解密为查询值交由 `verify_encrypted_search_receipt` 判定。类型非法抛
  `TypeError`，版本、范围、尺寸（含 `size >= 2**64`）、未知算法、摘要宽度或
  条目结构非法（含索引越出记录范围、非严格升序）抛 `ValueError`；规范二进制
  编解码由 `encode_encrypted_search_receipt` /
  `decode_encrypted_search_receipt` 提供
- `FullEncryptedSearchReceipt(version, hash_name, size, root, query, start,
  stop, items, proof, hits)` — 不可变的完整加密检索回执，把一次按原明文定位
  加密条目的**全覆盖**结果（算法、快照尺寸与 Merkle 根、规范化查询值、半开
  范围 `[start, stop)`、该范围内每一个 `Entry`、一份共享紧凑批量证明与签发方
  当场用查询密钥逐条解封比对得出的命中索引）绑成一件可落盘制品，按全部十个
  字段相等、支持位置构造；`version` 恒为 `1`，`query` 只收 `bytes` 或 `str`
  （后者按 UTF-8 规范化后存储），范围满足 `0 <= start <= stop <= size`，
  `items` 按绝对索引严格升序且恰为 `start .. stop-1` 逐条各一份（覆盖不全、
  重复或乱序均非法），每个条目的 `payload` 仍是密封的加密封装，`proof` 为
  覆盖全部所列条目的共享紧凑批量证明（与 `batch_inclusion_proof` 同一布局，
  空范围时与 `items`、`hits` 同为 `()`），`hits` 为严格升序且落在记录范围内
  的非 `bool` 整数元组；回执不记录密钥或明文，核验方认证条目后自行解封比对
  即得完整命中集并与 `hits` 逐项核对。`query` 以外的二进制字段（`root`、每个
  条目的 `payload` / `previous_hash` / `entry_hash` 与每个证明节点）只接受
  精确的 `bytes`，`bytearray` / `memoryview` 抛 `TypeError`。类型非法抛
  `TypeError`，版本、范围、未知算法、摘要宽度、条目结构、覆盖或命中索引非法
  （含空范围携带非空证明、命中越出范围或非严格升序）抛 `ValueError`；本层暂不
  提供规范二进制编解码
...
- `SignedFullSearchReceipt(receipt, checkpoint)` — 不可变的可信完整检索回执
  交付包，按两个字段相等、支持位置构造；`receipt` 为 `full_search_receipt` 的
  `FullSearchReceipt`（必须是 `FullSearchReceipt`，其自身结构契约由该类
...
- `AuditLog(*, key=None, hash_name="sha256")` — 传入非空 `bytes` 类型 `key` 开启前向安全认证，
  ...
  - `full_search_receipt(query, start=None, stop=None, size=None)` — 把一次按内容
    查找固化为可离线核验**完整性**的冻结 `FullSearchReceipt`：在与 `find` 相同的
    半开范围 `[start, stop)`（默认 `[retain_from, size)`）内，`size` 默认当前长度
    且快照须可重建；回执记录算法、快照尺寸与 Merkle 根、规范化查询值、范围、该
    范围内**每一个**条目（按绝对索引严格升序逐条各一份）以及一份覆盖全部条目的
    共享紧凑批量证明（空范围与空快照时条目与证明均为 `()`）。`query` 只收 `bytes`
    或 `str`（后者按 UTF-8 规范化）；查询值、范围或尺寸类型非法抛 `TypeError`，
    范围或尺寸越界、快照不可重建抛 `ValueError`；签发为只读、可重复调用，不改变
    条目、`head`、认证状态、Merkle 根或证明
  - `encrypted_search_receipt(query, key, start=None, stop=None, size=None)` —
    把一次 `find_encrypted` 的结果固化为可离线核验的冻结
    `EncryptedSearchReceipt`：沿用 `find_encrypted` 的口径，在半开范围
    `[start, stop)`（默认 `[retain_from, size)`）内定位，`size` 默认当前长度且
    快照须可重建；回执记录算法、快照尺寸与 Merkle 根、规范化查询值、范围与命中
    条目（每个命中是 `(Entry, 包含证明)` 对，`payload` 仍是密封加密封装，按
    绝对索引严格升序，重复明文逐次列出，无命中与空快照为 `()`），不记录密钥与
    明文。`query` 只收 `bytes` 或 `str`（后者按 UTF-8 规范化），其他类型抛
    `TypeError`；`key` 只收 32 字节 `bytes`（类型错抛 `TypeError`，长度不符抛
    `ValueError`）；范围或尺寸类型错抛 `TypeError`、越界或快照不可重建抛
    `ValueError`；异钥追加的条目与普通条目不命中；签发为只读，不改变条目、
    `head`、认证状态、密文定位索引、Merkle 根或证明
  - `full_encrypted_search_receipt(query, key, start=None, stop=None, size=None)` —
    把一次按原明文定位加密条目的查找固化为可离线核验**完整性**的冻结
    `FullEncryptedSearchReceipt`：在半开范围 `[start, stop)`（默认
    `[retain_from, size)`）内，`size` 默认当前长度且快照须可重建；回执记录算法、
    快照尺寸与 Merkle 根、规范化查询值、范围、该范围内**每一个**条目（按绝对
    索引严格升序逐条各一份，`payload` 仍是密封加密封装）、一份覆盖全部条目的
    共享紧凑批量证明，以及签发方当场用查询密钥逐条解封比对得出的升序命中索引
    （空范围与空快照时条目、证明与命中均为 `()`），不记录密钥与明文。`query`
    只收 `bytes` 或 `str`（后者按 UTF-8 规范化），其他类型抛 `TypeError`；
    `key` 只收 32 字节 `bytes`（类型错抛 `TypeError`，长度不符抛 `ValueError`）；
    范围或尺寸类型错抛 `TypeError`、越界或快照不可重建抛 `ValueError`；普通条目、
    异钥封装与解密失败都不算命中；签发为只读、可重复调用，不改变条目、`head`、
    认证状态、密文定位索引、Merkle 根或证明
  ...
  - `signed_search_receipt(query, private_key, start=None, stop=None, size=None)` —
    只读、可重复签发可信检索回执交付包，返回不可变 `SignedSearchReceipt`：先调用
    `search_receipt(query, start, stop, size)`（范围默认保留段、`size` 默认当前
    长度，语义与默认值沿用既有检索签发），再调用 `sign_root(private_key, size)`，
    据此构造 `SignedSearchReceipt(receipt, checkpoint)`，两部分描述同一可重建
  ...
  - `signed_full_search_receipt(query, private_key, start=None, stop=None,
    ...
    `SignedFullSearchReceipt`：先调用
    `full_search_receipt(query, start, stop, size)`（范围默认保留段、`size`
    默认当前长度，语义与默认值沿用完整检索签发），再调用
...
- `verify_full_search_receipt(receipt)` — 无需持有日志即可验证
  ...
  每一个条目（覆盖不全、重复或乱序在构造端即被拒绝），核验方自行比对
  `entry.payload == receipt.query` 即得范围内**完整**命中集——少列条目是结构
  错误而非静默遗漏。空范围（`items == ()`、`proof == ()`）照常核验，空快照
  （`size == 0`）只接受规范空树根。入参不是 `FullSearchReceipt` 抛 `TypeError`；
  字段类型、版本、范围、算法、摘要宽度、索引顺序、覆盖或证明节点数等结构非法
  （含绕过冻结构造器写入的字段）抛 `TypeError` / `ValueError`，与构造端一致；
  结构合法但条目内容、证明或根不符返回 `False`，匹配返回 `True`
- `verify_encrypted_search_receipt(receipt, key)` — 无需持有日志即可验证
  `EncryptedSearchReceipt`：逐条重算每个命中条目的 `entry_digest`、核验其包含
  证明是否落在记录的快照根内，再以 `key` 用 `decrypt_entry` 解密密封封装并把
  恢复明文与规范化查询值逐字节比较；只判定列出的命中是否属实，不判定结果集是否
  完整（少列条目不算失败，空命中与空快照照常核验）。密钥错误、AEAD 认证失败、
  列出普通条目或明文不符均返回 `False` 而不抛异常；入参不是
...
- `verify_full_encrypted_search_receipt(receipt, key)` — 无需持有日志即可验证
  `FullEncryptedSearchReceipt`：逐条重算每个所列条目的 `entry_digest`，再用那份
  共享紧凑批量证明（`verify_batch_inclusion`）核对记录的快照根（空快照只认规范
  空树根），然后以 `key` 用 `decrypt_entry` 逐条尝试解封，成功且恢复明文等于
  规范化查询值者计为命中；由于回执携带被搜范围内每一个条目（覆盖不全、重复或
  乱序在构造端即被拒绝），重算出的命中集就是范围内**完整**命中集，与回执记录的
  `hits` 逐项一致且真实性成立才返回 `True`。普通条目、异钥封装与解密失败都不算
  命中也不判失败；篡改的内容、证明或根一律返回 `False` 而不抛异常；记录了命中
  的回执在错误密钥下返回 `False`，零命中回执不考验密钥、只按真实性判定。入参
  不是 `FullEncryptedSearchReceipt` 或 `key` 不是 `bytes` 抛 `TypeError`；密钥
  长度、版本、范围、尺寸、未知算法、摘要宽度、索引顺序、覆盖、证明节点数或
  命中索引等结构非法（含绕过冻结构造器写入的字段）抛 `ValueError`
...
- `verify_signed_search_receipt(bundle, public_key)` — 凭预先信任的 32 字节
  Ed25519 公钥离线验证 `AuditLog.signed_search_receipt` 签发的
  `SignedSearchReceipt` 可信检索回执交付包，无需持有日志：先调用
  `verify_search_receipt`（重验每个列出命中的摘要与包含证明对快照根）与
  `verify_signed_root`（核验检查点签名），并要求两部分的 `hash_name`、`size`、
  `root` 一致，三者皆为真才返回 `True`。公钥不受信任、两部分不一致，或
  条目 / 证明 / 根 / 签名被改返回 `False`。入参不是 `SignedSearchReceipt`
  （含绕过构造器的容器字段类型错）或公钥不是 `bytes` 抛 `TypeError`；嵌套的
...
- `verify_signed_encrypted_search_receipt(bundle, key, public_key)` — 凭追加
  ...
  调用只读
- `verify_signed_full_search_receipt(bundle, public_key)` — 凭预先信任的 32
  字节 Ed25519 公钥离线验证 `AuditLog.signed_full_search_receipt` 签发的
  `SignedFullSearchReceipt` 可信完整检索回执交付包，无需持有日志：先调用
  `verify_full_search_receipt`（重验被搜范围内每一个条目的摘要并用共享批量
  证明核对快照根）与 `verify_signed_root`（核验检查点签名），并要求两部分的
  `hash_name`、`size`、`root` 一致，三者皆为真才返回 `True`。公钥不受信任、
  两部分不一致，或条目 / 证明 / 根 / 签名被改返回 `False`（绝不抛异常）。
...
- `encode_signed_search_receipt(bundle)` / `decode_signed_search_receipt(data)` —
  可信检索回执交付包的规范二进制编码与解码，使 `SignedSearchReceipt` 可落盘、
  跨进程传输后继续凭预置信任的 Ed25519 公钥离线验真，且不新增签名原文：魔数
  `b"auditchain/signed-search-receipt/v1\0"` 开头，严格依次写 version=1（u64）、
  receipt blob、checkpoint blob（不允许省略、换序或附加字段）；每个 blob 为 u64
  字节长度前缀加原始字节，内容分别是既有 `encode_search_receipt` 与
  `encode_signed_root` 的完整规范字节，解码精确消费两个 blob 并分别交给既有
  解码器。前者只接受 `SignedSearchReceipt`（外层或两部分快照不一致抛
  `ValueError`、容器字段类型错抛 `TypeError`，嵌套错误沿用既有编码器），后者只
  接受 `bytes`（拒绝 `bytearray` / `memoryview`）；魔数、版本、截断、尾随、blob
  长度或嵌套格式非法抛 `ValueError`；解码对象字段相等、冻结且重编码逐字节相同，
...
- `encode_signed_full_search_receipt(bundle)` /
  ...
  预置信任的 Ed25519 公钥离线验真，且不新增签名原文：字节流以魔数
  `b"auditchain/signed-full-search/v1\0"`（含 NUL 结尾）开头，严格依次写
  version=1（u64）、receipt blob、checkpoint blob，顺序固定、禁止换序与
  尾随；每个 blob 为 u64 字节长度前缀加原始字节，内容分别是既有
  `encode_full_search_receipt` 与 `encode_signed_root` 的完整规范字节，解码
  精确消费两个 blob 并分别交给既有解码器。前者只接受
  `SignedFullSearchReceipt`（容器字段类型错抛 `TypeError`，两部分快照不
  一致抛 `ValueError`，嵌套错误沿用既有编码器），后者只接受精确的 `bytes`
  （拒绝 `bytearray` / `memoryview`，抛 `TypeError`）；魔数、版本、截断、
  尾随、blob 长度越界、两部分快照不一致或任一嵌套格式非法抛 `ValueError`；
...
- `AnchorSet(items)` — 冻结的多包锚定续接链集合，把分批落盘的非空
  `AnchoredContinuationChain` 包 tuple 绑为一件可持久化、可跨进程恢复
  的制品，不再携带别的字段；支持位置/关键字构造、按该字段相等（可哈希）；
  `items` 非 tuple 或元素非 `AnchoredContinuationChain` 抛 `TypeError`，
  空集抛 `ValueError`；制品本身不校验包内验真与包间锚点接缝
- `encode_anchor_set(bundle)` /
  `decode_anchor_set(data)` — 多包锚定续接链集合的规范二进制编码与解码，
  使若干包作为单一制品落盘、跨进程恢复后把 `items` 交给既有
  `inspect_anchor_set` / `merge_anchor_set` 继续诊断或合并，全程只读、
  不新增签名域：字节流严格为 `D || U(1) || U(n) || B(P0)…B(Pn-1)`，
  其中 `D = b"auditchain/anchor-set/v1\0"`，`U` 为 8 字节无符号大端
  整数，`B(x) = U(len(x)) || x`，`n` 为包数，`Pi` 逐字节为既有
  `encode_anchored_continuations(items[i])` 输出，按 tuple 顺序排列且
...
- `encode_full_search_receipt(receipt)` / `decode_full_search_receipt(data)` —
  完整检索回执的规范二进制编码与解码，使 `FullSearchReceipt` 可落盘、跨进程恢复
  后继续由 `verify_full_search_receipt` 离线核验（编解码只读且确定）：魔数
  `b"auditchain/full-search/v1\0"`（含 NUL 结尾）开头，后接恒为 1 的 `version`，
  随后严格按回执字段顺序写 `hash_name`（UTF-8 blob）、`size`、`root` blob、
  `query` blob、`start`、`stop` 与条目计数，禁止换序与尾随；整数为 u64 大端，
  blob 为 u64 长度前缀加原始字节（零长度也是全零 u64），每个条目依次写
  `Entry.index`、`payload` blob、`previous_hash` blob、`entry_hash` blob（与
  `encode_audit_receipt` 的条目写法相同），最后写共享证明的节点计数及各摘要
  blob。解码精确消费全部字节，恢复的回执按全部字段与原件相等、重编码逐字节
  相同；空范围与空快照照常往返且核验通过，内容被篡改的回执仍可往返、仅核验判
  `False`。前者只接受 `FullSearchReceipt` 并复验全部字段与证明节点计数，后者
...
- `encode_encrypted_search_receipt(receipt)` /
  ...
  `verify_encrypted_search_receipt` 持密钥离线核验（编解码只读且确定）：魔数
  `b"auditchain/encrypted-search/v1\0"`（含 NUL 结尾）开头，后接恒为 1 的
  `version`，随后严格按回执字段顺序写 `hash_name`（UTF-8 blob）、`size`、
  `root` blob、`query` blob、`start`、`stop` 与命中计数；除魔数外字段布局与
  `encode_search_receipt` 完全一致，每个命中依次写 `Entry.index`、`payload`
  blob（密封加密封装原样写入）、`previous_hash` blob、`entry_hash` blob、
  证明计数及各摘要 blob，整数为 u64 大端，blob 为 u64 长度前缀加原始字节
  （零长度也是全零 u64），禁止换序与尾随。解码精确消费全部字节，恢复对象冻结、
  按全部字段与原件相等、重编码逐字节相同；空结果与空快照照常往返，内容被篡改的
  回执仍可往返、仅核验判 `False`。前者只接受 `EncryptedSearchReceipt` 并复验
  全部字段与证明层数，后者只接受精确的 `bytes`（拒绝
