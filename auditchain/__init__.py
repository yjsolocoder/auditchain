...
# fixed magic, then the envelope version as a u64 (always 1), the verdict as
# a u64 (0 success, 1 failure), the issue count as a u64 and, per issue, the
# issue code as a u64-length-prefixed UTF-8 blob followed by the position as
# a u64 (0 for the index-less "head" issue), in that strict order and with
# nothing else. It carries no signatures, only the frozen chain-verification
# diagnosis, and introduces no new signing message.
...
def encode_integrity_report(report: Any) -> bytes:
    ...
    issue count and one record per issue — nothing may be omitted, reordered
    or appended, and no signature is carried. Every integer is an unsigned
    8-byte big-endian value and every blob is a u64 byte length followed by
    the raw bytes (a zero length is an all-zero u64). The verdict is ``0``
    for success and ``1`` for failure: a success report writes an issue
    count of ``0``, while a failure writes one record per issue in the
    report's own ascending order (by absolute index, ``"index"`` /
    ``"previous_hash"`` / ``"entry_hash"`` at one position, the
    entry-independent ``"head"`` issue last). Each record is the issue code
    as a u64-length-prefixed UTF-8 blob — one of the four legal codes —
    followed by the position as a u64; the index-less ``"head"`` issue
    writes position ``0``.

    ``report`` must be an :class:`IntegrityReport` — anything else raises
    TypeError; fields written with wrong types by bypassing the frozen
    constructor (``object.__setattr__``) likewise raise TypeError, because
    every field is re-validated through the constructor. An issue position
    outside the u64 range raises ValueError. The call is read-only and
    deterministic: it never mutates the report, and re-encoding a decoded
    report reproduces the original bytes exactly, so a chain-verification
    diagnosis can be landed on disk and restored in another process with the
    same verdict and issues.
    """
    ...
    # with object.__setattr__, so corrupted fields raise exactly as the
    # constructor would: TypeError for wrong types, ValueError for illegal
    # values, ordering or an ok/issues mismatch. Each issue is rebuilt first
    # so its own fields are validated before the container's ordering check
    # reads them.
    ...
    for issue in report.issues:
        if not isinstance(issue, IntegrityIssue):
            raise TypeError("each issue must be an IntegrityIssue")
        rebuilt.append(IntegrityIssue(issue.code, issue.index))
    checked = IntegrityReport(report.ok, tuple(rebuilt))
    parts = [
...
def decode_integrity_report(data: Any) -> IntegrityReport:
    ...
    order, the u64 version (only ``1`` is supported), the u64 verdict
    (``0`` success, ``1`` failure), the u64 issue count and one record per
    issue — the code as a u64-length-prefixed UTF-8 blob followed by the
    u64 position — consumed whole with no trailing bytes. A success verdict
    must carry a zero issue count; a failure verdict must carry at least one
    issue. Each code must be one of ``"index"``, ``"previous_hash"``,
    ``"entry_hash"``, ``"head"``; the ``"head"`` record must write position
    ``0`` (decoded back to ``None``), and the records must follow the
    report's ascending order with a ``"head"`` record only last. A bad magic
    or version, a verdict other than 0/1, a success/failure issue-count
    mismatch, an unknown code, a code/position mismatch, a wrong order,
    truncation, trailing bytes, an oversized blob length or invalid UTF-8
    all raise ValueError.

    The returned object is a frozen :class:`IntegrityReport` whose fields
    equal the originally encoded report's; re-encoding it reproduces the
    original bytes exactly, and its verdict is the same before and after the
    round trip.
    """
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
    ...
    if verdict not in (0, 1):
        raise ValueError(
            f"integrity-report verdict must be 0 or 1, got {verdict}"
        )
    issue_count = read_u64("issues count")
    issues = []
    ...
    if verdict == 1 and not issues:
        raise ValueError(
            "a failed integrity report must carry at least one issue"
        )
    return IntegrityReport(ok=verdict == 0, issues=tuple(issues))
