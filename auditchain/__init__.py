...
# Binary framing of encode_prune_receipt / decode_prune_receipt: a fixed
# magic, then the format version and size as u64 and the hash name plus both
# digests as u64-length-prefixed blobs.
...
def encode_prune_receipt(receipt: Any) -> bytes:
    ...
    ``b"auditchain/prune-receipt/v1\\0"``; every integer is an unsigned 8-byte
    big-endian value and every blob is a u64 byte length followed by the raw
    bytes (a zero length is an all-zero u64). Fields appear in the order
    ``version`` (always 1), ``hash_name`` (UTF-8 blob), ``size``,
    ``merkle_root`` blob and ``chain_hash`` blob. ``receipt`` must be a
    :class:`PruneReceipt` — anything else, or a receipt whose fields have
    been bypassed to wrong types, raises TypeError; an unknown hash
    algorithm, a digest of the wrong width or a ``size`` outside the u64
    range raises ValueError. The call is read-only and deterministic: it
...
def decode_prune_receipt(data: Any) -> PruneReceipt:
    ...
    invalid UTF-8 in ``hash_name``, an unknown hash algorithm or one without
    a fixed-length digest, truncation, trailing bytes, an oversized blob
    length, or a ``merkle_root``/``chain_hash`` whose width does not match
    the named digest raises ValueError. The decoded receipt is a frozen
    :class:`PruneReceipt` whose fields equal the originally encoded ones;
    re-encoding reproduces the original bytes exactly, and the receipt can
    drive :meth:`AuditLog.prune` on a log of the same hash algorithm
    whenever the original could.
    """
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
    ...
    if version != _PRUNE_RECEIPT_VERSION:
        raise ValueError(f"unsupported prune receipt version {version}")
    raw_name = read_blob("hash_name")
    try:
        hash_name = raw_name.decode("utf-8")
    except UnicodeDecodeError as error:
