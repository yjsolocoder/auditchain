...
# Ed25519-signed counterpart of the hybrid framing, mirroring dump_signed_auth
# exactly — the wire form is D || U(version) || P || S: D is the magic,
# version the u64 envelope version (always 1), P field-by-field exactly the
# plaintext framing of dump_hybrid
# (B(hash_name) || U(q) || B(nonce1)…B(nonceq) || U(n) || E1…En ||
# B(root) || B(head) || U(stage) || B(K) || U(x), each Ei in the secure-log
# record order U, B, B, B, B with B(locator) driving plain/encrypted
# classification) and S the 64-byte Ed25519 signature over every preceding
# byte; no further signing domain is introduced and no key is encrypted. As
# with dump_signed_auth the signature authenticates the export's origin but
# not its confidentiality — P carries the live evolution key — so the byte
# string must be protected like key material.
...
def dump_signed_hybrid(log: Any, private_key: Any) -> bytes:
    ...
    the hybrid framing symmetrically, the export is signed with the 32-byte
    Ed25519 seed ``private_key`` and verified offline by
    :func:`load_signed_hybrid` against the pre-arranged public key. Only an
    :class:`AuditLog` that holds its complete, unpruned history
    (``retain_from == 0``) and was **constructed with an authentication key**
    qualifies; unlike :func:`dump_signed_auth`, the history may contain
    entries appended with :meth:`AuditLog.encrypt`. The framing carries the
    current forward-secure evolution key and stage in the clear, plus the
    encrypted-locator HMACs and the log's complete nonce history exactly as
    :func:`dump_hybrid` does, so a fresh process can continue evolving from
    exactly the same point and the restored log keeps its encrypted search
    index and rejects a reused nonce exactly as the original did.

    The encoding starts with the magic
    ``b"auditchain/signed-hybrid/v1\\0"`` and then writes, strictly in order,
    the envelope version (always ``1``) as an unsigned 8-byte big-endian
    integer, then exactly the plaintext framing of :func:`dump_hybrid` —
    ``B(h) || U(q) || B(nonce1)…B(nonceq) || U(n) || E1…En || B(root) ||
    B(head) || U(stage) || B(K) || U(x)``, where ``h`` is the UTF-8 encoding
    of the hash algorithm name, ``q`` the number of nonces ever used by
    :meth:`AuditLog.encrypt` in this log (each written as a 12-byte blob in
    lexicographic order), ``n`` the entry count, ``root`` the Merkle root and
    ``head`` the chain head of the size-``n`` snapshot, ``stage`` the current
    key-evolution stage, ``K`` the current evolution key and ``x`` the
    verifier-exported flag (``0`` or ``1``); each ``Ei`` reuses the
    :func:`dump_secure_log` record encoding
    ``U(index) || B(payload) || B(previous_hash) || B(entry_hash) ||
    B(locator)``, where an empty locator blob marks a plain entry and a
    non-empty locator is the digest-width encrypted-locator HMAC — and
    finally a 64-byte Ed25519 signature over every preceding byte. ``U`` is
    an unsigned 8-byte big-endian integer and ``B(v) = U(len(v)) || v``. No
    signing domain beyond the signed bytes themselves is introduced, and the
    evolution key is not encrypted, so the byte string must be protected
    like key material.
    ...
    or added as a separate signing input. A non-:class:`AuditLog` value or a
    non-``bytes`` seed raises TypeError; a seed that is not 32 bytes, or a
    log that is pruned or was constructed without an authentication key
    raises ValueError.
    """
    if not isinstance(log, AuditLog):
        raise TypeError("log must be an AuditLog")
    # Validate the seed before any eligibility check, mirroring
    # dump_signed_auth; the loaded key signs once below and is never stored.
    signing_key = _load_ed25519_seed(private_key)
    # Same eligibility as dump_hybrid, minus the symmetric transport: the
    # framing carries the live evolution key, stage, complete nonce history
    # and per-entry locators, so only an unpruned log that was constructed
    # with a key qualifies. An encrypt history is exactly what the nonce
    # history and locators preserve.
    if log._retain_from != 0:
        raise ValueError(
            "only an unpruned log holding its complete history can be dumped"
        )
...
def load_signed_hybrid(data: Any, public_key: Any) -> AuditLog:
    ...
    in order, the u64 envelope version (only ``1`` is supported), exactly the
    plaintext framing of :func:`dump_hybrid` —
    ``B(h) || U(q) || B(nonce1)…B(nonceq) || U(n) || E1…En || B(root) ||
    B(head) || U(stage) || B(K) || U(x)`` — and a final 64-byte Ed25519
    signature, with no truncation or trailing bytes.
    ...
    Verification happens before any state is built: the trailing signature
    is checked against every preceding byte with ``public_key``; only then
    is the framing parsed. Every nonce blob must carry exactly 12 bytes and
    the ``q`` nonces must be distinct and in lexicographic order; each
    ``Ei`` is an :class:`Entry` record in the :func:`dump_secure_log` field
    order (``U, B, B, B, B``) whose trailing ``B(locator)`` is empty for a
    plain entry and the digest-width encrypted-locator HMAC for an encrypted
    one; indices must be exactly ``0..n-1``, every chain digest must have the
    width of the named hash algorithm, ``stage`` must be a u64, ``K`` (the
    current evolution key) must be non-empty — at stage 0 it is the
    construction key, which may have any non-zero length, and after any
    evolution it is one hash-digest wide — and ``x`` (the verifier-exported
    flag) must be ``0`` or ``1``. Under the named ``hash_name`` every
    :func:`entry_digest` and predecessor link is then recomputed from
    genesis, and the resulting chain head and Merkle root must match
    ``head`` / ``root``. Classification is driven by the locator, never the
    payload: a non-empty locator's payload must parse as an encrypted-entry
    envelope, whose 12-byte nonce is recovered and must not repeat, and the
    recovered envelope nonces must together be exactly the declared nonce
    history. Only then is a fresh keyed :class:`AuditLog` built by replaying
    the payloads through the normal append / encrypted-entry recovery path,
    with the nonce history, ``K`` as the current key, ``stage`` and the
    exported flag installed, so its length, absolute indices, head, Merkle
    root, find index, encrypted locator index, nonce history and stage are
    exactly those of the dumped log and forward-secure evolution continues
    from the same point; an already-exported log cannot export stage-0
    verifier material again, and tags minted after the restore are
    byte-identical to the original's in the same state. The result is fully
    mutable and shares no state with the caller's buffers.

    A non-``bytes`` ``data`` or ``public_key`` raises TypeError; a public
    key that is not 32 bytes, a bad magic, version, UTF-8 or hash algorithm,
    truncation, trailing bytes, a wrong-width, duplicated or misordered
    nonce, a nonce history that disagrees with the encrypted entries, a
    wrong digest or locator width, an unparseable ciphertext envelope, an
    ...
    offset = len(_SIGNED_HYBRID_MAGIC)
    ...
    # payloads into a fresh keyed log so every auxiliary structure (find
    # index, encrypted locator index, nonce history, frontier) is rebuilt
    # exactly as the normal append / encrypt paths build it. The nonce
    # history, evolution key, stage and verifier-exported flag are only
    # installed after the replay succeeds, so a failure leaves no partially
    # restored log behind.
    log = AuditLog(key=evolution_key, hash_name=hash_name)
