...
def _issue(indices=(0, 2, 4), seed=_SEED_A, size=None, records=5, evolve=1, **kwargs):
    return _log(records=records, evolve=evolve, **kwargs).signed_stage_auth_audit_bundle(
        indices, seed, size
    )
...
class SignedStageAuthAuditBundleConstructionTest(unittest.TestCase):
    ...
    def test_constructor_does_not_validate_halves_internals(self):
        # Only the container shape is checked: a well-typed but content-free
        # pair constructs without error; verification is where it is judged.
        bundle = _issue(indices=())
        self.assertEqual(
            SignedStageAuthAuditBundle(bundle.auth, bundle.audit), bundle
        )
...
class SignedStageAuthAuditBundleIssuanceTest(unittest.TestCase):
    ...
    def test_audit_appends_unselected_last_entry_without_auth_tag(self):
        ...
        # The appended last entry carries no auth tag: exactly one tag and
    ...
    def test_auth_half_is_byte_for_byte_existing_stage_batch(self):
        log = _log(records=6, evolve=2)
        bundle = log.signed_stage_auth_audit_bundle((4, 1, 3), _SEED_A, size=5)
        twin = _log(records=6, evolve=2)
        standalone = twin.signed_stage_auth_bundle((4, 1, 3), _SEED_A)
        self.assertEqual(bundle.auth, standalone)
        # Deep byte equality of the delivered material and every tag.
        self.assertEqual(
            bundle.auth.verifier.signature, standalone.verifier.signature
        )
        self.assertEqual(
            [tag.tag for _, tag in bundle.auth.items],
            [tag.tag for _, tag in standalone.items],
        )

    def test_audit_half_uses_existing_batch_audit_semantics(self):
        log = _log(5)
        bundle = log.signed_stage_auth_audit_bundle((1,), _SEED_A, size=4)
        twin = _log(5)
        audit = twin.signed_audit_batch((1,), _SEED_A, 4)
        self.assertEqual(bundle.audit, audit)

    def test_tags_run_from_delivery_stage_and_commit(self):
        log = _log(records=5, evolve=2)
        ...
        # Success advanced the stage exactly once per selected entry.
    ...
    def test_empty_selection_delivers_material_without_stage_move(self):
        log = _log(5)
        stage = log.stage
    ...
    def test_empty_selection_consumes_no_one_shot_eligibility(self):
        ...
        # Repeated deliveries likewise leave the flag untouched.
        log.signed_stage_auth_audit_bundle((), _SEED_A)
        self.assertFalse(log._verifier_exported)
    ...
    def test_empty_snapshot(self):
        ...
        self.assertEqual(bundle.auth.items, ())
        hash_name, size, _root, entries, proof = bundle.audit.batch
    ...
    def test_same_state_and_seed_issue_byte_identical_bundles(self):
        one = _log(5).signed_stage_auth_audit_bundle((4, 0, 2), _SEED_A, size=5)
        two = _log(5).signed_stage_auth_audit_bundle((4, 0, 2), _SEED_A, size=5)
        self.assertEqual(one, two)
        self.assertEqual(one.auth.verifier.signature, two.auth.verifier.signature)
        self.assertEqual(
            one.audit.checkpoint.signature, two.audit.checkpoint.signature
        )
        self.assertEqual(
            [tag.tag for _, tag in one.auth.items],
            [tag.tag for _, tag in two.auth.items],
        )
    ...
    def test_out_of_snapshot_index_raises_index_error(self):
        log = _log(5)
        with self.assertRaises(IndexError):
            log.signed_stage_auth_audit_bundle([5], _SEED_A)
        with self.assertRaises(IndexError):
    ...
    def test_failure_is_atomic(self):
        # The unrebuildable-snapshot case only exists on a pruned log: a
        # failure there must likewise leave key, stage, tags and entries
        # untouched and sign nothing.
        pruned = _log(6)
        pruned.prune(2, pruned.seal(2))
        before = self._snapshot(pruned)
        with self.assertRaises(ValueError):
            pruned.signed_stage_auth_audit_bundle((), _SEED_A, size=1)
        self.assertEqual(self._snapshot(pruned), before)
        ...
        for call, error in (
            ...
            with self.assertRaises(error):
                call(log)
            self.assertEqual(self._snapshot(log), before)
            # No key evolution happened: the retry delivers the same stage.
...
class VerifySignedStageAuthAuditBundleTest(unittest.TestCase):
    ...
    def test_genuine_bundle_verifies(self):
        ...
        # The two halves individually verify as well.
        self.assertEqual(
            verify_signed_stage_auth_bundle(bundle.auth, self.public_key),
            (True, True, True),
        )
        self.assertTrue(verify_signed_audit_batch(bundle.audit, self.public_key))

    def test_empty_selection_still_checks_the_stage_verifier_signature(self):
        # No per-item results can pass vacuously: the signed stage material
        # itself must verify.
        bundle = _issue(indices=())
        self.assertTrue(
            verify_signed_stage_auth_audit_bundle(bundle, self.public_key)
    ...
    def test_tampered_tag_returns_false_without_raising(self):
        bundle = _issue()
    ...
    def test_tampered_proof_returns_false_without_raising(self):
        bundle = _issue(indices=(0, 2), records=5)
        hash_name, size, root, entries, proof = bundle.audit.batch
        self.assertTrue(proof)
        bad_node = bytes([proof[0][0] ^ 1]) + proof[0][1:]
        forged_batch = (hash_name, size, root, entries, (bad_node,) + proof[1:])
        forged_audit = SignedAuditBatch(forged_batch, bundle.audit.checkpoint)
        forged = SignedStageAuthAuditBundle(bundle.auth, forged_audit)
        self.assertFalse(
            verify_signed_stage_auth_audit_bundle(forged, self.public_key)
        )
    ...
    def test_authenticated_index_missing_from_audit_returns_false(self):
        auth = _log(5).signed_stage_auth_bundle((2,), _SEED_A)
        wider = _log(6)
    ...
    def test_algorithm_mismatch_between_packages_returns_false(self):
        auth = _log(3, hash_name="sha256").signed_stage_auth_bundle((0,), _SEED_A)
    ...
    def test_hash_name_disagrees_with_signed_verifier_returns_false(self):
        bundle = _issue()
        forged_auth = SignedStageAuthBundle(
            bundle.auth.verifier, "sha512", bundle.auth.items
        )
        forged = SignedStageAuthAuditBundle(forged_auth, bundle.audit)
        self.assertFalse(
            verify_signed_stage_auth_audit_bundle(forged, self.public_key)
    ...
    def test_bypassed_container_fields_raise_type_error(self):
        ...
        for field, value in (
            ...
            forged = SignedStageAuthAuditBundle.__new__(SignedStageAuthAuditBundle)
