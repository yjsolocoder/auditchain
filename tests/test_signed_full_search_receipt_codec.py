...
class EncodeSignedFullSearchReceiptTest(unittest.TestCase):
    ...
    def test_nested_receipt_value_error_propagates(self):
        ...
        # index/size is rejected by encode_full_search_receipt's ValueError.
...
class DecodeSignedFullSearchReceiptTest(unittest.TestCase):
    ...
    def test_roundtrip_empty_range(self):
        self.roundtrip(
            self.log.signed_full_search_receipt("hit", _SEED_A, 2, 2)
        )
    ...
    def test_tampered_payload_roundtrips_but_verifies_false(self):
        # A content-altered package still round-trips; only verification
        # reports False.
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A)
        receipt = bundle.receipt
        ...
        tampered_receipt = FullSearchReceipt(
            receipt.version,
            receipt.hash_name,
            receipt.size,
            receipt.root,
        ...
        forged = SignedFullSearchReceipt(tampered_receipt, bundle.checkpoint)
        decoded = decode_signed_full_search_receipt(
            encode_signed_full_search_receipt(forged)
        )
        self.assertEqual(decoded, forged)
