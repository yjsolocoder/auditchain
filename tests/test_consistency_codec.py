...
def build(hash_name, old_size, old_root, new_size, new_root, proof, version=1):
    ...
    out += u64(old_size) + blob(old_root) + u64(new_size) + blob(new_root)
    out += u64(len(proof))
    for node in proof:
        out += blob(node)
    return out
...
class ConsistencyProofTestBase(unittest.TestCase):
    ...
    def make_credential(self, old_size=3, new_size=None):
        new_size = len(self.log) if new_size is None else new_size
...
class ConsistencyProofConstructorTest(ConsistencyProofTestBase):
    def test_fields_in_order_and_positional_construction(self):
        ...
        self.assertEqual(credential.new_size, 7)
        self.assertEqual(credential.new_root, self.log.merkle_root())
        self.assertEqual(credential.proof, self.log.consistency_proof(3, 7))
    ...
    def test_frozen(self):
        ...
        with self.assertRaises(Exception):
            credential.new_size = 3

    def test_hash_name_checked(self):
        base = self.make_credential()
        with self.assertRaises(TypeError):
            ConsistencyProof(
                1, base.old_size, base.old_root, base.new_size, base.new_root,
                base.proof,
            )
    ...
    def test_sizes_checked(self):
        ...
        for bad, exc in (
            ...
            with self.assertRaises(exc):
                ConsistencyProof(
                    "sha256", bad, base.old_root, base.new_size, base.new_root,
                    base.proof,
                )
            with self.assertRaises(exc):
                ConsistencyProof(
                    "sha256", base.old_size, base.old_root, bad, base.new_root,
                    base.proof,
                )
        with self.assertRaises(ValueError):
            ConsistencyProof(
                "sha256", 5, base.old_root, 4, base.new_root, base.proof
            )
    ...
    def test_roots_checked(self):
        ...
        with self.assertRaises(TypeError):
            ConsistencyProof(
                "sha256", 3, bytearray(base.old_root), 7, base.new_root, base.proof
            )
        with self.assertRaises(ValueError):
            ConsistencyProof(
                "sha256", 3, b"\x00" * 31, 7, base.new_root, base.proof
            )
        with self.assertRaises(TypeError):
            ConsistencyProof(
                "sha256", 3, base.old_root, 7, bytearray(base.new_root), base.proof
            )
        with self.assertRaises(ValueError):
            ConsistencyProof(
                "sha256", 3, base.old_root, 7, b"\x00" * 31, base.proof
            )
    ...
    def test_proof_checked(self):
        ...
        with self.assertRaises(TypeError):
            ConsistencyProof(
                "sha256", 3, base.old_root, 7, base.new_root, list(base.proof)
            )
        with self.assertRaises(TypeError):
            ConsistencyProof(
                "sha256",
        ...
        with self.assertRaises(ValueError):
            ConsistencyProof(
                "sha256", 3, base.old_root, 7, base.new_root, (b"\x00" * 31,)
            )

    def test_proof_node_count_not_checked_by_constructor(self):
        # Node-count fit and root linking belong to verify_consistency.
        base = self.make_credential()
...
class DecodeConsistencyProofTest(ConsistencyProofTestBase):
    ...
    def test_roundtrip_variants(self):
        for old_size, new_size in (
            ...
            self.roundtrip(self.make_credential(old_size, new_size))
    ...
    def test_roundtrip_empty_proof(self):
        self.roundtrip(self.make_credential(7, 7))
    ...
    def test_unknown_hash_algorithm(self):
        base = self.make_credential()
        data = build(
            b"not-a-hash", base.old_size, base.old_root, base.new_size,
            base.new_root, base.proof,
        )
        with self.assertRaises(ValueError):
    ...
    def test_invalid_utf8_hash_name(self):
        base = self.make_credential()
        data = build(
            b"\xff\xfe", base.old_size, base.old_root, base.new_size,
            base.new_root, base.proof,
        )
        with self.assertRaises(ValueError):
            decode_consistency_proof(data)
    ...
    def test_digest_widths_checked(self):
        ...
        with self.assertRaises(ValueError):
            decode_consistency_proof(
                build(b"sha256", base.old_size, b"\x00" * 31, base.new_size,
                      base.new_root, base.proof)
            )
        with self.assertRaises(ValueError):
            decode_consistency_proof(
                build(b"sha256", base.old_size, base.old_root, base.new_size,
                      b"\x00" * 31, base.proof)
            )
        if base.proof:
            with self.assertRaises(ValueError):
    ...
    def test_size_order_rejected(self):
        base = self.make_credential()
        with self.assertRaises(ValueError):
            decode_consistency_proof(
                build(b"sha256", 7, base.old_root, 3, base.new_root, base.proof)
            )
    ...
    def test_wrong_root_decodes_but_verifies_false(self):
        base = self.make_credential()
        data = build(
            b"sha256", base.old_size, base.old_root, base.new_size,
            b"\x11" * 32, base.proof,
        )
        decoded = decode_consistency_proof(data)
