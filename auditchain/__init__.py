...
# holding the hash_name UTF-8 bytes and the key, strictly in Verifier field
# order — with nothing else (no stage field: a Verifier is always stage 0).
...
def encode_verifier(material: Any) -> bytes:
    ...
    The encoding starts with the magic
    ``b"auditchain/verifier/v1\\0"``; every integer is an unsigned
    8-byte big-endian value and every blob is a u64 byte length followed by
    the raw bytes (a zero length is an all-zero u64). Fields appear strictly
    in the order ``version`` (always 1), ``hash_name`` (UTF-8 blob) and
    ``key`` blob — nothing may be omitted, reordered or appended, and there
    is no stage field (a :class:`Verifier` is always stage 0). ``material``
    must be a :class:`Verifier` — anything else, or a material whose fields
    have been bypassed to wrong types, raises TypeError; an empty key or an
    unknown hash algorithm raises ValueError. The encoding carries the
    stage-0 key in the clear and must be protected exactly like the
    in-memory material. The call is read-only and deterministic: it never
    mutates the material, and re-encoding a decoded one reproduces the
    original bytes exactly.
    """
    ...
    checked = Verifier(material.key, material.hash_name)
    return b"".join((
        _VERIFIER_MAGIC,
...
def decode_verifier(data: Any) -> Verifier:
    ...
    ``memoryview``, raises TypeError). A bad magic, a version other than 1,
    invalid UTF-8 in ``hash_name``, an unknown hash algorithm, truncation,
    trailing bytes, an oversized blob length or an empty key raises
    ValueError. The decoded material's fields equal the originally encoded
    ones, re-encoding reproduces the original bytes exactly, and it works
    unchanged with :func:`verify_auth` and :func:`verify_auth_batch`,
    reaching the same verdicts on the same tags; the material is frozen and
    compares equal to the original on all fields.
    """
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
