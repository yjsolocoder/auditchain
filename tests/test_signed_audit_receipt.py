...
class SignedAuditReceiptIssueTest(unittest.TestCase):
    ...
    def test_last_entry_auto_added(self):
        ...
        indices = [entry.index for entry, _proof in bundle.receipt.items]
        self.assertEqual(indices, [0, 4])
        last_entry = bundle.receipt.items[-1][0]
        self.assertEqual(last_entry.entry_hash, self.log.head)
        self.assertEqual(bundle.checkpoint.head, last_entry.entry_hash)
        self.assertTrue(verify_signed_audit_receipt(bundle, self.public_key))

    def test_per_entry_proofs_match_audit_receipt(self):
        bundle = self.log.signed_audit_receipt([0, 2, 4], _SEED_A)
        self.assertEqual(bundle.receipt, self.log.audit_receipt([0, 2, 4]))
        for entry, proof in bundle.receipt.items:
            self.assertEqual(
    ...
    def test_positional_construction_and_equality(self):
        ...
        self.assertNotEqual(
            bundle, (bundle.receipt, bundle.checkpoint)
        )
    ...
    def test_constructor_field_types(self):
        ...
        for bad_receipt, bad_checkpoint in (
            ...
            with self.assertRaises(TypeError):
                SignedAuditReceipt(bad_receipt, bad_checkpoint)

    def _genuine_parts(self, size=5):
        receipt = self.log.audit_receipt([0, 2], size)
    ...
    def test_constructor_rejects_parts_of_different_snapshots(self):
        ...
        with self.assertRaises(ValueError):
            SignedAuditReceipt(receipt, other_checkpoint)
        other_receipt = self.log.audit_receipt([0, 2], 4)
        with self.assertRaises(ValueError):
            SignedAuditReceipt(other_receipt, checkpoint)
    ...
    def test_constructor_rejects_different_hash_algorithm(self):
        ...
        with self.assertRaises(ValueError):
            SignedAuditReceipt(receipt, foreign)
...
class VerifySignedAuditReceiptTest(unittest.TestCase):
    ...
    def test_nested_receipt_type_errors_propagate(self):
        ...
        with self.assertRaises(TypeError):
            verify_signed_audit_receipt(forged, self.public_key)
    ...
    def test_cross_check_is_algorithm_size_root_only(self):
        # The bundle-level cross-check is exactly hash_name/size/root, per
        # the delivery contract: a checkpoint signed over the genuine
        # algorithm, size and root but a different head field still carries a
        # valid signature over its own message, so the bundle verifies; the
        # genuine chain head is independently anchored by the receipt's last
        # carried entry.
        checkpoint = self.bundle.checkpoint
    ...
    def test_empty_snapshot_pairs_canonical_root(self):
        ...
        self.assertTrue(
            verify_signed_audit_receipt(empty, self.public_key)
        )
