...
MAGIC = b"auditchain/signed-full-search/v1\0"
...
class EncodeSignedFullSearchReceiptTest(unittest.TestCase):
    def setUp(self):
        self.log = AuditLog()
    ...
    def test_magic_and_field_layout(self):
        ...
        # encode_full_search_receipt bytes: B(R).
        receipt_bytes = encode_full_search_receipt(bundle.receipt)
    ...
    def test_blobs_are_exact_existing_encodings(self):
        ...
        self.assertEqual(receipt_blob, encode_full_search_receipt(bundle.receipt))
        self.assertEqual(
            checkpoint_blob, encode_signed_root(bundle.checkpoint)
        )
        # The inner blobs are independently decodable by the existing codecs.
        self.assertEqual(decode_full_search_receipt(receipt_blob), bundle.receipt)
        self.assertEqual(
    ...
    def test_only_signed_full_search_receipt_accepted(self):
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A)
        for bad in (
            None,
            1,
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                encode_signed_full_search_receipt(bad)
    ...
    def test_bypassed_container_field_types_raise_type_error(self):
        ...
        for field, value in (
            ...
            forged = SignedFullSearchReceipt.__new__(SignedFullSearchReceipt)
            object.__setattr__(
                forged,
                "receipt",
            ...
            with self.assertRaises(TypeError, msg=field):
                encode_signed_full_search_receipt(forged)
    ...
    def test_nested_receipt_value_error_propagates(self):
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A)
        # A structurally-bypassed receipt whose proof node count does not fit
        # the listed indices and size is rejected by
        # encode_full_search_receipt's ValueError.
        ...
        object.__setattr__(
            forged_receipt, "proof", receipt.proof + (b"\x00" * 32,)
        )
        forged = SignedFullSearchReceipt.__new__(SignedFullSearchReceipt)
        object.__setattr__(forged, "receipt", forged_receipt)
        object.__setattr__(forged, "checkpoint", bundle.checkpoint)
    ...
    def test_call_is_read_only(self):
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A)
        before = encode_signed_full_search_receipt(bundle)
        encode_signed_full_search_receipt(bundle)
        self.assertEqual(encode_signed_full_search_receipt(bundle), before)
        self.assertTrue(
            verify_signed_full_search_receipt(bundle, self.public_key)
        )


class DecodeSignedFullSearchReceiptTest(unittest.TestCase):
    def setUp(self):
        self.log = AuditLog()
    ...
    def roundtrip(self, bundle):
        ...
        self.assertIsInstance(decoded, SignedFullSearchReceipt)
        ...
        self.assertIs(type(decoded.receipt), FullSearchReceipt)
        ...
        self.assertTrue(
            verify_signed_full_search_receipt(decoded, self.public_key)
        )
        return decoded
    ...
    def test_roundtrip_variants(self):
        for query, start, stop, size in (
            ...
            self.roundtrip(
                self.log.signed_full_search_receipt(query, _SEED_A, start, stop, size)
            )

    def test_roundtrip_empty_log(self):
        self.roundtrip(AuditLog().signed_full_search_receipt("hit", _SEED_A))
    ...
    def test_roundtrip_after_prune(self):
        ...
        self.roundtrip(self.log.signed_full_search_receipt("r4", _SEED_A))
    ...
    def test_roundtrip_alternate_hash(self):
        for hash_name in ("sha512", "sha3_256"):
            ...
            for record in ("a", "hit", "b"):
                log.append(record)
            self.roundtrip(log.signed_full_search_receipt("hit", _SEED_A))

    def test_frozen(self):
        data = encode_signed_full_search_receipt(
    ...
    def test_persistence_across_process_boundary(self):
        ...
        restored = decode_signed_full_search_receipt(bytes(data))
        self.assertEqual(restored, bundle)
        self.assertTrue(
            verify_signed_full_search_receipt(restored, self.public_key)
        )
    ...
    def test_only_bytes_accepted(self):
        ...
        for bad in (
            ...
            with self.assertRaises(TypeError, msg=repr(bad)):
                decode_signed_full_search_receipt(bad)
    ...
    def test_truncation(self):
        ...
        for cut in range(len(MAGIC), len(data)):
            with self.assertRaises(ValueError, msg=cut):
                decode_signed_full_search_receipt(data[:cut])
    ...
    def test_trailing_bytes(self):
        ...
        for extra in (b"\x00", b"trailing", b"\x00" * 8):
            with self.assertRaises(ValueError, msg=extra):
                decode_signed_full_search_receipt(data + extra)
    ...
    def test_swapped_blob_order_rejected(self):
        ...
        with self.assertRaises(ValueError):
            decode_signed_full_search_receipt(swapped)
    ...
    def test_nested_snapshot_mismatch_rejected(self):
        ...
        framed = (
            MAGIC
            + u64(1)
            + blob(encode_full_search_receipt(forged.receipt))
            + blob(encode_signed_root(forged.checkpoint))
        )
        with self.assertRaises(ValueError):
            decode_signed_full_search_receipt(framed)
    ...
    def test_bad_signature_still_decodes_but_verifies_false(self):
        ...
        forged = SignedFullSearchReceipt(bundle.receipt, bogus)
        decoded = decode_signed_full_search_receipt(
            encode_signed_full_search_receipt(forged)
        )
    ...
    def test_wrong_key_decodes_but_verifies_false(self):
        bundle = self.log.signed_full_search_receipt("hit", _SEED_B)
        decoded = decode_signed_full_search_receipt(
            encode_signed_full_search_receipt(bundle)
        )
        self.assertFalse(
        ...
        self.assertTrue(
            verify_signed_full_search_receipt(decoded, self.other_public_key)
        )

    def test_tampered_receipt_still_roundtrips_but_verifies_false(self):
        # A content-tampered bundle encodes, decodes and re-encodes exactly;
        # only verification reports the forgery.
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A)
        receipt = bundle.receipt
        ...
        forged = SignedFullSearchReceipt(forged_receipt, bundle.checkpoint)
        data = encode_signed_full_search_receipt(forged)
        decoded = decode_signed_full_search_receipt(data)
        self.assertEqual(decoded, forged)
        self.assertEqual(encode_signed_full_search_receipt(decoded), data)
        self.assertFalse(
    ...
    def test_call_is_read_only(self):
        bundle = self.log.signed_full_search_receipt("hit", _SEED_A)
        data = encode_signed_full_search_receipt(bundle)
        decode_signed_full_search_receipt(data)
        self.assertEqual(
            decode_signed_full_search_receipt(data), bundle
        )
