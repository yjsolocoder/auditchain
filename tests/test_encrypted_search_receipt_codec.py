...
def make_log(plaintexts=("a", "b", "a", "c", "a"), **kwargs):
    log = AuditLog(**kwargs)
    for nonce, plaintext in enumerate(plaintexts):
        log.encrypt(plaintext, KEY, nonce=bytes([nonce]) * 12)
    return log
...
class EncodeEncryptedSearchReceiptTest(unittest.TestCase):
    ...
    def make_bypassed(self, **fields):
        """Receipt with invalid structure, bypassing the frozen validation."""
        receipt = self.log.encrypted_search_receipt("a", KEY)
...
class DecodeEncryptedSearchReceiptTest(unittest.TestCase):
    ...
    def test_roundtrip_unicode_query(self):
        self.log.encrypt("位置主张", KEY, nonce=b"\xf0" * 12)
        self.log.encrypt("位置主张", KEY, nonce=b"\xf1" * 12)
    ...
    def test_roundtrip_after_prune(self):
        self.log.encrypt("b", KEY, nonce=b"\xf2" * 12)
    ...
    def test_only_bytes_accepted(self):
        data = encode_encrypted_search_receipt(
    ...
    def test_proof_structure_checked(self):
        ...
        for bad_proof in (proof[:-1], proof + (b"\x00" * 32,)):
            with self.assertRaises(ValueError):
                decode_encrypted_search_receipt(
                    encode_encrypted_search_receipt(self.make_bypassed(((entry, bad_proof),)))
                )
