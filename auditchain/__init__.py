...
_SIGNED_ENCRYPTED_SEARCH_RECEIPT_MAGIC = (
    b"auditchain/signed-encrypted-search-receipt/v1\0"
)
_SIGNED_ENCRYPTED_SEARCH_RECEIPT_VERSION = 1
...
class SignedEncryptedSearchReceipt:
    """Keyed-search receipt over encrypted entries, sealed by a pre-trusted key.
    ...
    only the 32-byte append key and a pre-trusted 32-byte Ed25519 public key
    can confirm in one artifact that the listed encrypted-search hits with
    their inclusion proofs, the snapshot Merkle root and the chain head were
    all issued by the log holder — without holding the :class:`AuditLog`:
    ...
    whether the listed envelopes decrypt to the query and whether the
    checkpoint signature is genuine is left to
...
class AuditLog:
    ...
    def signed_encrypted_search_receipt(
        ...
        receipt over encrypted entries, sealed by a pre-trusted Ed25519 key.
...
def verify_signed_encrypted_search_receipt(
    ...
    decrypts each sealed envelope with the 32-byte append ``key``, and
    :func:`verify_signed_root` verifies the checkpoint signature with the
    32-byte ``public_key``. The two parts are then required to describe the
    same snapshot — equal ``hash_name``, ``size`` and ``root``. A genuine
    sealed bundle from the trusted key returns True; a structurally valid
    bundle signed by another key, whose parts disagree, whose entries,
    proofs, root or signature have been altered, or whose envelopes do not
    decrypt to the query under ``key`` returns False. Verification only
    attests that the listed hits are genuine — it makes no claim about
    whether the result set is complete, so a bundle listing fewer hits than
    the log would have found is not a failure, and a wrong append key simply
    yields False.

    Input that is not a :class:`SignedEncryptedSearchReceipt` (or whose
    container fields have been bypassed to wrong types) raises TypeError;
    nested structural violations raise exactly the exceptions of
    :class:`EncryptedSearchReceipt` and :func:`verify_signed_root` (TypeError
    or ValueError), a ``key`` that is not ``bytes`` raises TypeError and one
    that is not 32 bytes raises ValueError, as does a public key that is not
    32 ``bytes`` (a non-``bytes`` public key raises TypeError). The call is
...
def encode_signed_encrypted_search_receipt(bundle: Any) -> bytes:
    ...
    The encoding starts with the magic
    ``b"auditchain/signed-encrypted-search-receipt/v1\\0"``; it then writes,
    strictly in order, the envelope ``version`` (always 1) as an unsigned
    8-byte big-endian integer, the receipt blob and the checkpoint blob —
    nothing may be omitted, reordered or appended. Each blob is a u64 byte
    length followed by the raw bytes: the receipt blob is the complete
    canonical output of :func:`encode_encrypted_search_receipt` over
    ``bundle.receipt`` and the checkpoint blob is the complete canonical
    output of :func:`encode_signed_root` over ``bundle.checkpoint``. No new
    signing message is introduced: encoding is read-only and only re-uses the
    existing canonical encodings.
    ...
    deterministic: re-encoding a decoded bundle reproduces the original bytes
    exactly, and a structurally valid bundle whose signature does not match
    ...
    return b"".join((
        _SIGNED_ENCRYPTED_SEARCH_RECEIPT_MAGIC,
        _encode_u64(_SIGNED_ENCRYPTED_SEARCH_RECEIPT_VERSION, "version"),
        _encode_blob(receipt_blob),
        _encode_blob(checkpoint_blob),
    ))
...
def decode_signed_encrypted_search_receipt(data: Any) -> SignedEncryptedSearchReceipt:
    ...
    ``data`` must be ``bytes`` (anything else, including ``bytearray`` and
    ``memoryview``, raises TypeError). After the magic
    ``b"auditchain/signed-encrypted-search-receipt/v1\\0"`` it must contain,
    strictly in order, the u64 envelope version (only ``1`` is supported),
    one length-prefixed receipt blob and one length-prefixed checkpoint blob,
    with no trailing bytes. Each blob is handed whole to the existing decoder
    — :func:`decode_encrypted_search_receipt` and :func:`decode_signed_root`
    respectively — so every nested framing and structural rule is theirs. A
    bad magic or version, truncation, an oversized blob length, trailing
    bytes or an illegal nested encoding raises ValueError, as does a decoded
    pair whose receipt and checkpoint do not describe the same snapshot.
    ...
    if not isinstance(data, bytes):
        raise TypeError("data must be bytes")
    if not data.startswith(_SIGNED_ENCRYPTED_SEARCH_RECEIPT_MAGIC):
        raise ValueError(
            "not an auditchain signed-encrypted-search-receipt encoding"
        )
    offset = len(_SIGNED_ENCRYPTED_SEARCH_RECEIPT_MAGIC)
    ...
    version = read_u64("version")
    if version != _SIGNED_ENCRYPTED_SEARCH_RECEIPT_VERSION:
        raise ValueError(
            f"unsupported signed-encrypted-search-receipt version {version}"
        )
    receipt_blob = read_blob("receipt")
    checkpoint_blob = read_blob("checkpoint")
    if offset != len(data):
